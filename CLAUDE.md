...
### Calls / VOIP (`calls2`)
The `calls2` package is a from-WASM reimplementation of WA Web/Desktop 1:1 and group (SFU) voice/video calls; it supersedes the older, incorrect `call` package. Native FFM bindings live in `modules/lib/natives`; the pure-Java MLow codec (WA's `smpl` float CELP audio codec) is under `calls2/media/audio/mlow`. Cobalt-side layers: signaling (`Calls2LifecycleController`, `OfferStanza`, `signaling/*`), relay/media transport (`net/transport/*`: `LiveRelayTransport`, `MediaTransport`, `VoipDtlsTransport`, `E2eMediaSrtp`), the media session (`core/LiveMediaSession`, streams/subscriptions), and the driver example run from IntelliJ (`WebQrLoginExample`).
