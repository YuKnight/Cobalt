package com.github.auties00.cobalt.call.toolkit.input;

import com.github.auties00.cobalt.call.io.AudioFrame;
import com.github.auties00.cobalt.call.io.AudioSource;
import com.github.auties00.cobalt.call.toolkit.ffmpeg.bindings.AVChannelLayout;
import com.github.auties00.cobalt.call.toolkit.ffmpeg.bindings.AVCodecContext;
import com.github.auties00.cobalt.call.toolkit.ffmpeg.bindings.AVCodecParameters;
import com.github.auties00.cobalt.call.toolkit.ffmpeg.bindings.AVFormatContext;
import com.github.auties00.cobalt.call.toolkit.ffmpeg.bindings.AVFrame;
import com.github.auties00.cobalt.call.toolkit.ffmpeg.bindings.AVPacket;
import com.github.auties00.cobalt.call.toolkit.ffmpeg.bindings.AVStream;
import com.github.auties00.cobalt.call.toolkit.ffmpeg.bindings.Ffmpeg;
import com.github.auties00.cobalt.call.toolkit.ffmpeg.FFmpegError;
import com.github.auties00.cobalt.call.toolkit.ffmpeg.FFmpegLoader;
 ...
 */
public final class AudioFile implements AudioSource, AutoCloseable {
    /**
    ...
    public AudioFile(Path path) {
        ...
        try {
            ...
            int codecId = AVCodecParameters.codec_id(params);
     ...
     */
    private void pump() {
        int read = Ffmpeg.av_read_frame(formatCtx, packet);
        if (read < 0) {
        ...
        try {
            int idx = AVPacket.stream_index(packet);
            if (idx != streamIndex) {
                return;
            }
            int sent = Ffmpeg.avcodec_send_packet(codecCtx, packet);
    ...
    private void drainDecoder(boolean flush) {
        while (true) {
            int got = Ffmpeg.avcodec_receive_frame(codecCtx, frame);
    ...
    private void resampleAndQueue() {
        int inSamples = AVFrame.nb_samples(frame);
        if (inSamples <= 0) {
            return;
        }
        int outCapacity = (int) (((long) inSamples * OUT_SAMPLE_RATE
        ...
        try (var local = Arena.ofConfined()) {
            ...
            int produced = Ffmpeg.swr_convert(swrCtx, outPtrs, outCapacity, inPtrs, inSamples);
            if (produced < 0) {
    ...
    private void flushResampler() {
        try (var local = Arena.ofConfined()) {
            int outCapacity = OUT_FRAME_SAMPLES * 4;
            var outBuf = local.allocate((long) outCapacity * Short.BYTES);
            var outPtrs = local.allocate(ValueLayout.ADDRESS);
            outPtrs.set(ValueLayout.ADDRESS, 0L, outBuf);
            int produced = Ffmpeg.swr_convert(swrCtx, outPtrs, outCapacity, MemorySegment.NULL, 0);
            if (produced < 0) {
    ...
    private void queueResampled(MemorySegment outBuf, int produced) {
        int total = leftover.length + produced;
        var combined = new short[total];
        System.arraycopy(leftover, 0, combined, 0, leftover.length);
        var nativeShorts = outBuf.asByteBuffer().order(java.nio.ByteOrder.nativeOrder()).asShortBuffer();
        nativeShorts.get(combined, leftover.length, produced);

        int chunks = total / OUT_FRAME_SAMPLES;
        ...
        int remainder = total - chunks * OUT_FRAME_SAMPLES;
        if (remainder > 0) {
    ...
    private static int pickAudioStream(MemorySegment formatCtx) {
        int n = AVFormatContext.nb_streams(formatCtx);
    ...
    private static MemorySegment buildResampler(Arena arena, MemorySegment codecCtx) {
        ...
        int srcFmt = AVCodecContext.sample_fmt(codecCtx);
        int srcRate = AVCodecContext.sample_rate(codecCtx);
