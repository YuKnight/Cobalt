 ...
 * </ul>
 */
public final class Tone implements AudioSource {
    /**
     ...
     */
    public Tone(double freq1Hz, double freq2Hz, long onFrames, long cadenceFrames,
    ...
    public static Tone ringback() {
        long onFrames = 1000 / DEFAULT_FRAME_DURATION_MS;
        long cadenceFrames = 5000 / DEFAULT_FRAME_DURATION_MS;
        return new Tone(425, 0, onFrames, cadenceFrames,
                DEFAULT_FRAME_SIZE, DEFAULT_FRAME_DURATION_MS);
    }
    ...
    public AudioFrame next() {
        ...
        long pts = ptsMs.getAndAdd(frameDurationMs);
        boolean inOnPhase = (index % cadenceFrames) < onFrames;
        var pcm = new short[frameSize];
        if (!inOnPhase) {
            return new AudioFrame(pcm, pts);
        }
        long sampleBase = index * frameSize;
        double w1 = 2 * Math.PI * freq1Hz / SAMPLE_RATE;
        double w2 = 2 * Math.PI * freq2Hz / SAMPLE_RATE;
        boolean dual = freq2Hz > 0;
        double scale = dual ? AMPLITUDE / 2 : AMPLITUDE;
        for (int i = 0; i < frameSize; i++) {
            double t = sampleBase + i;
            double sample = Math.sin(w1 * t);
            if (dual) {
