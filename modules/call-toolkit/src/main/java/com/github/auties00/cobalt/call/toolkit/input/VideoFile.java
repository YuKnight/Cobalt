...
import com.github.auties00.cobalt.call.toolkit.ffmpeg.bindings.AVRational;
 ...
 */
public final class VideoFile implements VideoSource, AutoCloseable {
    /**
    ...
    private VideoFrame convertCurrentFrame() {
        int w = AVFrame.width(frame);
        int h = AVFrame.height(frame);
        int srcFmt = AVFrame.format(frame);
        ...
        try (var local = Arena.ofConfined()) {
            ...
            int produced = Ffmpeg.sws_scale(swsCtx,
        ...
        long ptsRaw = AVFrame.best_effort_timestamp(frame);
        long ptsMs = (timeBaseDen == 0)
                ? 0L
