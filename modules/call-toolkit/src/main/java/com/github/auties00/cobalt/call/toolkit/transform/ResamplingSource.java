 ...
 * encoder.
 */
public final class ResamplingSource implements AudioSource {
    /**
     ...
     *                                  is not positive
     */
    public ResamplingSource(AudioSource delegate, int inSampleRate, int outSampleRate, int outFrameSize) {
    ...
    public AudioFrame next() throws InterruptedException {
        ...
        int neededInputSamples = (int) Math.ceil((double) outFrameSize * inSampleRate / outSampleRate);
        ...
        int available = inBuffer.length - inBufferPos;
        if (available <= 0) {
            return null;
        }
        var out = new short[outFrameSize];
        for (int i = 0; i < outFrameSize; i++) {
            double inIndex = (double) i * inSampleRate / outSampleRate;
            int floor = (int) Math.floor(inIndex);
            double frac = inIndex - floor;
            int idx0 = inBufferPos + floor;
            int idx1 = idx0 + 1;
            int s0 = idx0 < inBuffer.length ? inBuffer[idx0] : 0;
            int s1 = idx1 < inBuffer.length ? inBuffer[idx1] : s0;
    ...
    private void appendInput(short[] more) {
        int unread = inBuffer.length - inBufferPos;
