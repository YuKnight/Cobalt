...
public final class PixelFormats {
    ...
    private static VideoFrame convertInterleaved(byte[] src, int width, int height, long ptsMs,
        ...
        for (int row = 0; row < height; row++) {
            int srcRow = row * width * stride;
            int yRow = row * width;
            for (int col = 0; col < width; col++) {
                int p = srcRow + col * stride;
                int r = src[p + rOff] & 0xFF;
                int g = src[p + gOff] & 0xFF;
                int b = src[p + bOff] & 0xFF;
                int y = ((CY_R * r + CY_G * g + CY_B * b + 128) >> 8) + 16;
        ...
        for (int row = 0; row < uvHeight; row++) {
            for (int col = 0; col < uvWidth; col++) {
                int srcRow0 = (row * 2) * width * stride;
                int srcRow1 = (row * 2 + 1) * width * stride;
                int srcCol0 = (col * 2) * stride;
                int srcCol1 = (col * 2 + 1) * stride;
                int rSum = (src[srcRow0 + srcCol0 + rOff] & 0xFF)
                        + (src[srcRow0 + srcCol1 + rOff] & 0xFF)
                        + (src[srcRow1 + srcCol0 + rOff] & 0xFF)
                        + (src[srcRow1 + srcCol1 + rOff] & 0xFF);
                int gSum = (src[srcRow0 + srcCol0 + gOff] & 0xFF)
                        + (src[srcRow0 + srcCol1 + gOff] & 0xFF)
                        + (src[srcRow1 + srcCol0 + gOff] & 0xFF)
                        + (src[srcRow1 + srcCol1 + gOff] & 0xFF);
                int bSum = (src[srcRow0 + srcCol0 + bOff] & 0xFF)
                ...
                int gAvg = gSum >> 2;
                int bAvg = bSum >> 2;
                int u = ((CU_R * rAvg + CU_G * gAvg + CU_B * bAvg + 128) >> 8) + 128;
                int v = ((CV_R * rAvg + CV_G * gAvg + CV_B * bAvg + 128) >> 8) + 128;
                int uvIdx = row * uvWidth + col;
