package com.github.auties00.cobalt.call.toolkit.output;
...
public final class CallRecorder implements AutoCloseable {
    ...
    private void writePacket(MemorySegment stream, int streamIndex,
        ...
        try (var local = Arena.ofConfined()) {
            ...
            int num = AVRational.num(tb);
            int den = AVRational.den(tb);
            long ts = num == 0 ? ptsMs : (ptsMs * den) / (1000L * num);
            AVPacket.pts(packet, ts);
            AVPacket.dts(packet, ts);
            AVPacket.duration(packet, 0L);
            AVPacket.flags(packet, keyframe ? 1 : 0);

            int wrote = Ffmpeg.av_interleaved_write_frame(formatCtx, packet);
            if (wrote < 0) {
    ...
    public synchronized void close() {
        ...
        try (arena) {
            if (headerWritten && formatCtx != null && formatCtx.address() != 0L) {
                int trailer = Ffmpeg.av_write_trailer(formatCtx);
                if (trailer < 0) {
