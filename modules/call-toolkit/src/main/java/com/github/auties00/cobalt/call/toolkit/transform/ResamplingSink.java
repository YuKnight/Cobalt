 ...
 * of {@link ResamplingSource}, used when the call decoder emits
 * 16 kHz mono but the OS speaker wants 48 kHz.
 *
 * <p>Pure-Java linear-interpolation fallback.
 */
public final class ResamplingSink implements AudioSink {
    /**
     ...
     */
    public ResamplingSink(AudioSink delegate, int inSampleRate, int outSampleRate) {
    ...
    public void write(AudioFrame frame) throws InterruptedException {
        ...
        var in = frame.pcm();
        int outLen = (int) Math.round((long) in.length * outSampleRate / inSampleRate);
        ...
        for (int i = 0; i < outLen; i++) {
            ...
            int s1 = floor + 1 < in.length ? in[floor + 1] : s0;
