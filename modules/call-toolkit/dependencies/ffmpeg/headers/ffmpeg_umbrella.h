/*
 * Umbrella header that pulls in every FFmpeg public surface the
 * cobalt-calls-toolkit binds to. Used as the single input to
 * jextract so the whole transitive type graph (AVRational →
 * AVPacket → AVCodecParameters → AVCodec → AVStream →
 * AVFormatContext, plus the libsw* state objects) is visible in
 ...
 * those transitively from the function signatures we actually
 * bind.
 *
 * The corresponding generated class is `Ffmpeg.java`; the toolkit
 * wrappers reference it directly. Re-run dependencies/ffmpeg/generate.sh
