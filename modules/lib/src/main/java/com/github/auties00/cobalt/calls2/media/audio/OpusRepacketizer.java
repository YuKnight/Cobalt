 ...
 *
 * @implNote This implementation ports {@code opus_codec_parse} (fn6264) and the repacketizer wrappers
 * {@code wa_opus_repacketizer_init} (fn6281), {@code wa_opus_repacketizer_cat} (fn6282), and
 * {@code wa_opus_repacketizer_out_range_impl} (fn6284) of the wa-voip WASM module {@code ff-tScznZ8P}.
 * The repacketizer state is allocated once with {@code cobalt_opus_repacketizer_create} and
 * re-initialized per operation with {@code cobalt_opus_repacketizer_init}, as the native wrappers do; the
 * output buffer is sized for the worst-case six-frame packet. The portable shim allocates the state from
 * the libopus heap behind an opaque handle rather than exposing {@code opus_repacketizer_get_size} for a
 * caller-allocated buffer, so the binding carries no build-dependent state size. The native code caps the
 * frame count against an internal array bound and errors on an invalid packet length; this port enforces
 * the {@code 1..6} frame count before the native call.
 */
public final class OpusRepacketizer implements AutoCloseable {
     ...
     *
     * <p>Re-initializes the native repacketizer, appends each frame with
     * {@code cobalt_opus_repacketizer_cat}, and emits the combined packet with
     * {@code cobalt_opus_repacketizer_out_range}. A single-frame list is returned as-is after a defensive
     * copy, since aggregation of one frame is the frame itself.
     *
     ...
     *
     * <p>Re-initializes the native repacketizer over the packet, reads the frame count with
