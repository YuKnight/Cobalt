 ...
 * call, or a call-link join) rather than answer immediately, the wa-voip engine does not allocate a full
 ...
 *
 * @implNote This implementation ports the pending-call buffer (native {@code pending_call_ctx} at
 * {@code DAT_ram_001508c4}, a {@code 0x218}-byte context) and its handlers from the wa-voip WASM module
 * {@code ff-tScznZ8P}: {@code handle_pending_call_offer} (fn10959) allocates the pending context and
 * stores the offer, {@code fill_pending_call_group_info} (fn10960) fills the projected group info, the
 * message queue is filled by fn10961 and replayed when the user joins, and {@code reject_pending_call}
 * sets the pending state to its terminal value (the native {@code pending_ctx} state {@code 4}) and emits
 * the decline. This port keeps the offer and the queued {@link CallMessage} records directly rather than
 * cloning them into a buffer pool, because the records are immutable; the {@link State} enum models the
 * native pending-state byte, with {@link State#REJECTED} corresponding to the native terminal value.
 */
public final class Calls2PendingCall {
    /**
    ...
    public enum State {
         ...
         *
         * @implNote This implementation binds the native {@code pending_ctx} terminal state value
         * {@code 4} set by {@code reject_pending_call}.
         */
        REJECTED
    }
     ...
     */
    public Calls2PendingCall(OfferStanza offer) {
    ...
    public IncomingCall toIncomingCall(Instant receivedAt, boolean offlineOffer) {
        ...
        var peer = offer.callerPnValue().orElse(offer.callCreator());
        var groupJid = offer.groupJidValue().orElse(null);
        var chatJid = resolveChatJid(peer, groupJid);
        return new IncomingCall(offer.callId(), peer, chatJid, receivedAt, offer.isVideo(),
