...
/**
 * Seals and opens one direction of SFrame end-to-end media frames, driving the per-key-id cipher,
 * framing each ciphertext with the SFrame trailer, and enforcing replay protection.
 *
 * <p>The wire frame this transform produces is {@code [ciphertext][auth tag][keyId varint][counter
 * varint][length byte]}: the {@link SFrameCipher} body (ciphertext followed by the truncated
 * authentication tag) first, then the {@link SFrameHeaderCodec} trailer of the LEB128 key id and
 * counter closed by a final byte equal to the trailer's total length, so the decoder reads the last
 * byte to locate the trailer. The trailer bytes, including the length byte, are the associated data
 * the cipher authenticates, and the counter is the per-frame nonce selector.
 *
 * <p>An instance protects exactly one direction and is single-writer: the seal path stamps a
 ...
 *
 * @implNote This implementation reproduces {@code wa_sframe_encrypt} (fn6898) and
 * {@code wa_sframe_decrypt} (fn6900) and the {@code SecureFrameTransform} stat counters
 * ({@code wa_sframe_get_stats} fn6901) of the wa-voip WASM module {@code ff-tScznZ8P}. The encrypt
 * path resolves the cipher for the ratchet's key id, writes the trailer
 * ({@code [keyId][counter][len]}), encrypts with the trailer as associated data, appends the trailer
 * after the ciphertext-and-tag, and increments the frame/byte counters; the decrypt path reads the
 * trailer length from the final byte, decodes key id then counter, rejects a zero counter and
 * replays through the {@code 0x800}-slot window, verifies the HMAC then AES-CTR-decrypts, and marks
 * the counter consumed. The seal side uses a single key id (0) per direction, as the 1:1 call media
 * path does; a group rekey installs a new chain key under a new key id through the provider.
 */
public final class SFrameSecureFrame {
    /**
     * Holds the key provider resolving the per-key-id cipher for this direction.
     */
    ...
    public synchronized byte[] seal(byte[] plaintext) {
        ...
        var body = cipher.seal(trailer, plaintext, counter);
        var frame = Arrays.copyOf(body, body.length + trailer.length);
        System.arraycopy(trailer, 0, frame, body.length, trailer.length);
        totalFrames++;
    ...
    public synchronized byte[] open(byte[] frame) {
        ...
        var trailerBytes = Arrays.copyOfRange(frame, trailerStart, frame.length);
        var body = Arrays.copyOfRange(frame, 0, trailerStart);
        var plaintext = cipher.open(trailerBytes, body, counter);
        if (plaintext == null) {
            errorFrames++;
            return null;
        }
