 ...
 * its peer has pre-registered a receive context for; a stream arriving on any other SSRC is dropped as
 * an unknown source and never decoded. The set per device covers the audio and the two simulcast video
 * streams (each with a primary, an FEC, and an out-of-band NACK companion), the screen-share stream,
 * the hop-by-hop FEC streams, the AppData stream, and the IMU-data stream.
 *
 * <p>Each SSRC is the little-endian interpretation of a four-byte HKDF-SHA256 expansion keyed by the
 * shared call-id, salted by the four-byte media-type code, and contexted by the device identifier:
 *
 * {@snippet :
 ...
 * }
 *
 * <p>The call-id is the shared, on-the-wire identifier from the offer, reproducible from the call
 * signaling alone; it is not the secret call key. The device identifier is the same
 * {@code <lid>:<device>@lid} string that keys the device's media SRTP. Every method is stateless and
 * thread-safe. This class supersedes the legacy {@code call.rtp.srtp.SecureSsrcDerivation}.
 *
 * @implNote This implementation reproduces {@code generate_single_ssrc} (fn10913) and the per-device
 *           set composition of {@code call_generate_device_ssrc} (fn10901) and
 *           {@code call_generate_ssrc_for_identifier} (fn10902), which HKDF-expand the device identifier
 *           under the call-id keyed by the four-byte media-type code and read the result little-endian.
 *           It is verified byte-exact against a live call (call-id
 *           {@code 007498E578A915C0F9814AC2CB48D28F}): the audio main/fec/nack triple for
 *           {@code 258252122116273:94@lid} is {@code 0xE438BF63}/{@code 0xD3CB93DC}/{@code 0x6DE76824}
 *           and for {@code 39110693621863:0@lid} is {@code 0x3058A8C4}/{@code 0x4D2B0011}/{@code 0x5ABCB5F0}.
 *           A media-type code of zero salts with four zero bytes, which HMAC-SHA256 zero-pads identically
 *           to the RFC 5869 no-salt default. The HKDF is computed through {@link VoipCryptoNative}.
 */
public final class CallSecureSsrcGenerator {
    /**
     * Holds the media-type code of a receive (incoming) primary stream.
     *
     * @implNote This implementation reproduces the {@code 0} rx-primary code of
     *           {@code call_generate_ssrc_for_identifier} (fn10902).
     */
     ...
     *
     * @implNote This implementation reproduces the {@code 2} tx-primary code of
     *           {@code call_generate_ssrc_for_identifier} (fn10902).
     */
     ...
     *
     * @implNote This implementation reproduces the {@code 6} AppData code of {@code generate_app_data_ssrc}
     *           (fn10903).
     */
     ...
     *
     * @implNote This implementation reproduces the {@code 10} IMU code of {@code generate_imu_data_ssrc}
     *           (fn10904).
     */
     ...
     *
     * @implNote This implementation reproduces the two-stream simulcast count of
     *           {@code call_generate_device_ssrc} (fn10901).
     */
     ...
     *
     * <p>The HKDF input keying material is the call-id, the salt is the little-endian four-byte media-type
     * code, and the info is the device JID for stream id {@code 0} or the device JID followed by
     * {@code "_<streamId>"} for any higher stream id. The four-byte output is read little-endian.
     *
     * @param callId     the call-id string from the offer (the shared on-the-wire identifier)
     ...
     * @implNote This implementation reproduces {@code generate_single_ssrc} (fn10913); the legacy
     *           {@code call.rtp.srtp.SecureSsrcDerivation#ssrc(String, String, int, int)} computes the
     *           identical schedule.
     */
    public static int ssrc(String callId, CallDeviceJid deviceJid, int mediaType, int streamId) {
        ...
        // The native identifier is always "<user>:<device>@<server>" with the device present even for the
        // primary device 0, whereas Jid.toString() omits ":0" for the primary; build it explicitly so a
        // device-0 participant derives the same SSRC the peer pre-registers. Call participant JIDs are
        // agentless LID device JIDs, so no agent component appears.
     ...
     * {@link #MEDIA_TYPE_RX_FEC}, and {@link #MEDIA_TYPE_RX_OOB_NACK}. The live capture pins a device's
     * audio stream to this {@code 0}/{@code 1}/{@code 4} code family (a device's video streams use the
     ...
     *
     * @param callId    the call-id string from the offer
     ...
     * @implNote This implementation reproduces the {@code 0}/{@code 1}/{@code 4} branch of
     *           {@code call_generate_ssrc_for_identifier} (fn10902) used for the audio stream; the
     *           primary, FEC, and NACK values are verified live (caller audio
     *           {@code 0xE438BF63}/{@code 0xD3CB93DC}/{@code 0x6DE76824}).
     */
     ...
     * @throws NullPointerException       if {@code callId} or {@code deviceJid} is {@code null}
     * @throws WhatsAppCallException.Srtp if an HKDF computation fails
     * @implNote This implementation reproduces the {@code 2}/{@code 3}/{@code 5} branch of
     *           {@code call_generate_ssrc_for_identifier} (fn10902) used for the video streams; the
     *           values and the {@code "_1"} second-stream suffix are verified live (caller video stream 0
     *           {@code 0xA4AF8E8F}/{@code 0x2D89D462}/{@code 0x935DF37C}, stream 1
     *           {@code 0xCC5B7B35}/{@code 0xC8FE9CF6}/{@code 0xB49EB3A4}).
     */
     ...
     * @throws NullPointerException       if {@code callId} or {@code deviceJid} is {@code null}
     * @throws WhatsAppCallException.Srtp if the HKDF computation fails
     * @implNote This implementation reproduces the audio-primary derivation (code {@code 0}) and is
     *           verified live (caller audio main {@code 0xE438BF63}, peer audio main {@code 0x3058A8C4}).
     */
     ...
     * @throws NullPointerException       if {@code callId} or {@code deviceJid} is {@code null}
     * @throws WhatsAppCallException.Srtp if the HKDF computation fails
     * @implNote This implementation reproduces {@code generate_app_data_ssrc} (fn10903, media-type
     *           {@code 6}).
     */
     ...
     * @throws NullPointerException       if {@code callId} or {@code deviceJid} is {@code null}
     * @throws WhatsAppCallException.Srtp if the HKDF computation fails
     * @implNote This implementation reproduces {@code generate_imu_data_ssrc} (fn10904, media-type
     *           {@code 10}).
     */
     ...
     * @implNote This implementation models the SSRC triple produced by
     *           {@code call_generate_ssrc_for_identifier} (fn10902).
     */
