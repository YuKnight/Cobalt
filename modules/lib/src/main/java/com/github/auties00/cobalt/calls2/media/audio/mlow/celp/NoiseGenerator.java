...
/**
 * Generates the noise excitation that MLow adds to the deterministic pulse-plus-pitch excitation, the port
 * of {@code smpl_celp_gen_noise} and its {@code add_noise_uv} helper in {@code smpl_celp_util.c} together
 * with the pseudo-random pulse generator {@code smpl_gen_rand_pulses} from {@code smpl_codec_util.c}.
 *
 ...
 * the voiced/voiced-tail noise and a voicing-dependent high-pass auto-regressive moving-average stage for
 * the unvoiced noise, whose corner frequency rises when the first two line spectral frequencies crowd
 * together (a sign of a sharp formant).
 *
 * <p>This generator is stateful: it owns the smoothing states the native {@code NoiseGenerator} struct holds
 * (the envelope smoother, the previous envelope tail, the two output-filter states, the smoothed
 * autocorrelation, the shaping-filter state, the previous-voicing and since-unvoiced counters, and the
 * pseudo-random seed). Construct one per logical decode stream and call {@link #genNoise} once per subframe
 * in order; {@link #reset()} returns every state to the freshly constructed (zeroed) value, the equivalent of
 * the native {@code calloc}-zeroed struct.
 *
 * <p>Scope is the SMPL 16 kHz, 60 ms, mono low-band path with {@code SMPL_LPC_ORDER == 16}. The
 * profiling instrumentation macros ({@code TIC}/{@code TOC}) of the native function carry no semantics and
 * are dropped. This type is not thread-safe.
 *
 * @implNote This implementation reproduces the native single-precision arithmetic statement for statement,
 * including the {@code smpl_gen_rand_pulses} linear-congruential generator (seed update
 * {@code seed = 907633515 + seed * 196314165} with 32-bit overflow, four outputs per step taken from the
 * top, second, third, and fourth bytes of the new seed scaled by {@code 8.1e-10f}) and the
 * {@code smpl_powf_fast}-free transcendental calls ({@code expf}, {@code logf}, {@code sqrtf}) which map to
 * {@code Math.exp}/{@code Math.log}/{@code Math.sqrt} narrowed to {@code float}. The discrete-cosine
 * transform matrix and the spectral-factorization Newton iteration are computed exactly as
 * {@code smpl_create_celp_tables} and {@code smpl_spec_fact2} compute them. The float path is not bit-exact
 * against the C decoder (C double-precision intermediate promotion under {@code -Ofast} versus Java strict
 * single precision), but it tracks the C decoder's own noise within the carried float envelope; the
 * pseudo-random pulse sequence itself is bit-exact because it is pure 32-bit integer arithmetic.
 */
public final class NoiseGenerator {
    /**
     * Linear-prediction order of the MLow short-term filter, the native {@code SMPL_LPC_ORDER}; the length
     * of the line-spectral-frequency vector the unvoiced high-pass corner reads.
     */
    private static final int LPC_ORDER = 16;

    /**
     * The order of the short-term autocorrelation used to spectrally match the voiced noise, the native
     * {@code SMPL_NOISE_CORR_ORDER} = 2.
     */
    private static final int NOISE_CORR_ORDER = 2;

    /**
     * The number of discrete-cosine-transform bands the voiced autocorrelation is flattened over, the native
     * {@code SMPL_NOISE_DCT_ORDER} = 16.
     */
    private static final int NOISE_DCT_ORDER = 16;

    /**
     * Pi as a single-precision constant, the native {@code SMPL_PI}.
     */
    private static final float PI = 3.1415926535897f;

    /**
     * The voiced noise output gain, the native {@code SMPL_DEC_NOISE_V_NOISE_GAIN} = {@code 0.35f}.
     */
    private static final float DEC_NOISE_V_NOISE_GAIN = 0.35f;

    /**
     * The unvoiced noise output gain, the native {@code SMPL_DEC_NOISE_UV_NOISE_GAIN} = {@code 0.8f}.
     */
    private static final float DEC_NOISE_UV_NOISE_GAIN = 0.8f;

    /**
     * The base unvoiced high-pass corner frequency in Hertz, the native
     * {@code SMPL_DEC_NOISE_UV_FCORNER_HZ} = {@code 800.0f}.
     */
    private static final float DEC_NOISE_UV_FCORNER_HZ = 800.0f;

    /**
     * The voiced envelope smoothing coefficient, the native {@code SMPL_ENV_SMTH_COEF_V} = {@code 0.95f}.
     */
    private static final float ENV_SMTH_COEF_V = 0.95f;

    /**
     * The unvoiced envelope smoothing coefficient, the native {@code SMPL_ENV_SMTH_COEF_UV} = {@code 0.995f}.
     */
    private static final float ENV_SMTH_COEF_UV = 0.995f;

    /**
     * The voiced-onset crossfade decay coefficient, the native {@code SMPL_ENV_SMTH_COEF_UV_V} = {@code 0.99f}.
     */
    private static final float ENV_SMTH_COEF_UV_V = 0.99f;

    /**
     * The fixed second-order moving-average smoother applied to the voiced (and voiced-tail) noise, the
     * native {@code coef_ma_v} = {@code {0.25f, -0.496f, 0.25f}}.
     */
    private static final float[] COEF_MA_V = {0.25f, -0.496f, 0.25f};

    /**
     * The pseudo-random generator multiplier, the native {@code RAND_MULTIPLIER} = {@code 196314165}.
     */
    private static final int RAND_MULTIPLIER = 196314165;

    /**
     * The pseudo-random generator increment, the native {@code RAND_INCREMENT} = {@code 907633515}.
     */
    private static final int RAND_INCREMENT = 907633515;

    /**
     * The pseudo-random pulse output scale, the native {@code 8.1e-10f} factor in {@code smpl_gen_rand_pulses}.
     */
    private static final float RAND_SCALE = 8.1e-10f;

    /**
     * The shared discrete-cosine-transform matrix, the native {@code CelpTables.dct_mat_t}; row-major
     * {@code [SMPL_NOISE_CORR_ORDER + 1][SMPL_NOISE_DCT_ORDER]}, computed once at construction.
     */
    private final float[][] dctMatT;

    /**
     * The voiced envelope smoother state, the native {@code NoiseGenerator.env_smth}.
     */
    private float envSmth;

    /**
     * The previous subframe's last envelope value, the native {@code NoiseGenerator.env_last}.
     */
    private float envLast;

    /**
     * The unvoiced output high-pass filter state, the native {@code NoiseGenerator.out_state_uv}, two taps.
     */
    private final float[] outStateUv;

    /**
     * The voiced output moving-average smoother state, the native {@code NoiseGenerator.out_state_v}, two
     * taps.
     */
    private final float[] outStateV;

    /**
     * The smoothed short-term autocorrelation of the voiced excitation, the native
     * {@code NoiseGenerator.corr_smth}, {@code SMPL_NOISE_CORR_ORDER + 1} taps.
     */
    private final float[] corrSmth;

    /**
     * The voiced shaping-filter state, the native {@code NoiseGenerator.shape_state},
     * {@code SMPL_NOISE_CORR_ORDER} taps.
     */
    private final float[] shapeState;

    /**
     * The previous subframe's voicing flag, the native {@code NoiseGenerator.prev_voiced}.
     */
    private int prevVoiced;

    /**
     * The count of consecutive voiced subframes since the last unvoiced one, the native
     * {@code NoiseGenerator.since_unvoiced}; gates the voiced-tail unvoiced injection.
     */
    private int sinceUnvoiced;

    /**
     * The pseudo-random generator seed, the native {@code NoiseGenerator.rand_seed}.
     */
     ...
     * since-unvoiced counters, and the pseudo-random seed start at zero, mirroring the native
     * {@code calloc}-zeroed {@code NoiseGenerator} struct.
     */
    ...
    /**
     * Produces one subframe's noise excitation, {@code smpl_celp_gen_noise}.
     *
     ...
     *                          entries
     * @param l                 the subframe length in samples, the native {@code L}
     * @param voiced            {@code true} for the voiced shaping branch, {@code false} for the unvoiced
     *                          branch
     * @param numPulses         the subframe's pulse count, the native {@code num_pulses}; zero selects the
     *                          flat-envelope unvoiced sub-branch
     * @param nrgres            the target linear residual energy, the native {@code nrgres}
     * @param fcbgIdx           the subframe's unvoiced fixed-codebook gain index, used to cap the gapped
     *                          unvoiced noise
     * @param lsf               the subframe's line spectral frequencies, {@code SMPL_LPC_ORDER} entries; set
     *                          the unvoiced high-pass corner
     * @param normalizedBitrate the frame's normalized bitrate, the native {@code normalized_bitrate}
    ...
    public void genNoise(float[] excLpc, int l, boolean voiced, int numPulses, float nrgres,
        ...
        // noise_v2 is offset by 3 in the native code so the MA smoother can read its state; the smoother
        // state is held separately, so a plain L-length buffer suffices here.
    ...
    /**
     * Filters and adds the unvoiced noise burst into the output, {@code add_noise_uv}.
     *
     ...
     * @param lsf        the line spectral frequencies, {@code SMPL_LPC_ORDER} entries
     * @param nrgRatio   the excitation-to-residual energy ratio, the native {@code nrg_ratio}
    ...
    /**
     * Generates a pseudo-random bipolar pulse train, {@code smpl_gen_rand_pulses}.
     *
     ...
     * the byte extraction are pure 32-bit integer arithmetic, so this is bit-exact against the C generator.
     *
    ...
    /**
     * Computes the smoothed energy envelope of a signal, {@code smpl_get_env}.
     *
     * <p>Tracks a leaky-integrated short-term energy and writes its square root as the envelope, processing
     * four samples per step with the same two-rate update the native code uses. The smoother state carries
     * across subframes via {@link #envSmth}.
     *
    ...
    /**
     * Computes a decaying envelope from the smoother state alone, {@code smpl_get_env0}.
     *
    ...
    /**
     * Applies a second-order moving-average filter with carried state, {@code smpl_filt_ma2}.
     *
     * <p>Reproduces the native fast path: a monic-aware first stage plus the second tap, with the two-tap
     * state supplying the filter history at the subframe boundary. The native routine forbids aliasing the
     * input and output; this port writes into a separate output buffer, so the constraint is met.
     *
    ...
    /**
     * Applies a first-order auto-regressive moving-average filter with carried state, {@code smpl_filt_arma1}.
     *
     * <p>Runs the first-order moving-average stage then the first-order auto-regressive stage, with the
     * two-tap state holding one tap for each stage. The native routine uses a scratch buffer when the input
     * and output alias; the unvoiced caller does alias them, so this reproduces that by staging through a
     * temporary moving-average buffer.
     *
     * @param x      the input signal, {@code n} entries
    ...
    private static void filtArma1(float[] x, int n, float[] coefMa, float[] coefAr, float[] state, float[] y) {
        ...
        // First-order MA stage (smpl_filt_ma1).
        if (coefMa[0] == 1.0f) {
            ma[0] = x[0];
            for (int i = 1; i < n; i++) {
        ...
        state[0] = x[n - 1];
        // First-order AR stage (smpl_filt_ar1): y[n] = x[n] - coefAr[1] * y[n-1].
        float ar1 = -coefAr[1];
        float ytmp = state[1];
        for (int i = 0; i < n; i++) {
            ytmp = ma[i] + ytmp * ar1;
    ...
    /**
     * Computes the symmetric two-zero, two-pole spectral factorization, {@code smpl_spec_fact2}.
     *
     ...
     *
     * @param c the three target correlations; {@code c[0]} is non-negative and is biased in place
    ...
    /**
     * Multiplies the transpose of the discrete-cosine matrix by a vector, {@code smpl_matrix_mult_transp_16}.
     *
     * <p>Computes the {@code 16}-entry product {@code y = C^T x} where {@code C} is the row-major
     * {@code [lenX][16]} matrix; the result length is fixed at {@code SMPL_NOISE_DCT_ORDER}.
     *
    ...
    /**
     * Multiplies the discrete-cosine matrix by a vector, {@code smpl_matrix_mult}.
     *
    ...
    /**
     * Computes the dot product of two windowed signals, {@code smpl_dot_prod}.
     *
    ...
    /**
     * Computes the energy (sum of squares) of a signal, {@code smpl_nrg}.
     *
     * @param x the signal
     * @param n the length
    ...
    /**
     * Computes the sum of a signal, {@code smpl_sum}.
     *
     * @param x the signal
     * @param n the length
    ...
    /**
     * Returns the maximum of a signal, {@code smpl_maximum}.
     *
     * @param x   the signal
    ...
    /**
     * Computes one entry of the unvoiced fixed-codebook gain table and halves it for the noise cap, the
     * {@code pTbl->fcbgains_uv[fcbg_idx] * 0.5f} expression of {@code smpl_celp_gen_noise}.
     *
    ...
    /**
     * Computes the shared discrete-cosine-transform matrix, the {@code dct_mat_t} initialization of
     * {@code smpl_create_celp_tables}.
     *
     * <p>Entry {@code [j][i]} is {@code cos(j * (0.5 + i) * pi / SMPL_NOISE_DCT_ORDER) / sqrt(SMPL_NOISE_DCT_ORDER)},
     * the type-III cosine basis the voiced-noise spectral flattening projects onto and back from.
     *
