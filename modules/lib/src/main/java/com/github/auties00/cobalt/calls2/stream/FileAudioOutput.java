 ...
 *
 * <p>This is the device-backed {@link AudioOutput} returned by {@link AudioOutput#file(Path)}. Its
 * constructor opens the file, picks its first audio stream, decodes it, and resamples each decoded frame
 * to 16 kHz mono signed 16-bit PCM so the output matches the call media format. Any container or codec the
 * bundled FFmpeg build enables a decoder for is supported, which by default covers WAV, FLAC, MP3, AAC,
 * Opus, Vorbis, MP4, MKV, and OGG.
 *
 * <p>A background thread decodes the file ahead of the consumer into a bounded read-ahead buffer, and each
 * {@link #take()} returns one buffered 160-sample, 10 ms frame. Decoding ahead keeps the call's paced
 * sender supplied across demux-decode-resample jitter, which is otherwise worst at the codec's cold start
 * and would starve the sender into choppy playback. Sub-frame audio is never lost: the leftover samples
 * from a decode that does not land on a 10 ms boundary are carried into the next decode. When the file
 * ends, the trailing partial frame (if any) is dropped and {@link #take()} returns {@code null} to signal
 * end-of-stream. {@link #shutdown()} stops the decoder thread and releases the native demuxer, decoder,
 * and resampler.
 *
 * @implNote This implementation is the calls2 port of the legacy media-file capture stream; the per-frame
 ...
 */
public final class FileAudioOutput extends BufferedAudioOutput {
    /**
     ...
     * earlier than this origin plus {@code p}, pacing a file source to wall-clock real time the way a
    ...
    /**
     * Decodes the file ahead of the consumer, feeding {@link #prefetch} until the stream drains or shuts
     * down.
     *
     ...
     * pacing the file source to real time.
     *
     ...
     * {@inheritDoc}
     *
     * <p>Marks the source ended, stops and joins the background decoder so no FFmpeg call is in flight,
     * then frees the decoder output frame, the demuxer packet, the resampler, the decoder context, and the
     * demuxer context, and closes the owning arena. Guards each pointer against {@code null} and a zero
     * address, so the call is idempotent.
     */
    @Override
     ...
     *
     * <p>Reads one packet; on end-of-input it flushes the decoder and marks the stream drained. Otherwise
     * it feeds packets belonging to the chosen audio stream to the decoder and drains the resulting
     * frames.
     */
    private void pump() {
