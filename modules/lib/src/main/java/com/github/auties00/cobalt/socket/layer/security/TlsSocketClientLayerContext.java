...
public final class TlsSocketClientLayerContext implements SocketClientLayerContext {
    ...
    /**
     * The socket channel used by this TLS connection, captured during
     * the handshake for sending the {@code close_notify} alert on
     * disconnect.
     */
    ...
    /**
     * Wraps outbound application data into a TLS record in
     * {@link #netOutBuffer} without writing to a channel.
     *
     * <p>Used by the dual-TLS write path (HTTPS proxy) where the
     * transport TLS wraps plaintext into its {@code netOutBuffer}, which
     * the tunnel TLS then wraps and writes to the channel.  The returned
     * buffer is {@link #netOutBuffer} itself in read mode — the caller
     * must consume it (or pass it to the next wrap) before this context
     * is used again.
     *
     * @param buffers the application data buffers
     * @param offset  the offset into the buffers array
     * @param count   the number of buffers to wrap
     * @return {@link #netOutBuffer} in read mode containing the
     *         encrypted TLS record, or {@code null} if nothing was
     *         produced
     * @throws IOException if a TLS error occurs
     */
    ...
    /**
     * Sends the TLS {@code close_notify} alert to the peer.
     *
     * <p>After {@link SSLEngine#closeOutbound()}, one final
     * {@link SSLEngine#wrap(ByteBuffer, ByteBuffer) wrap} produces
     * the {@code close_notify} alert record.  This method writes that
     * record to the channel on a best-effort basis — if the channel
     * is already closed or the write fails, the error is silently
     * ignored since the connection is being torn down.
     *
     * @implNote RFC 8446 Section 6.1 requires each party to send a
     *           {@code close_notify} alert before closing its write side.
     */
    private void sendCloseNotify() {
        ...
        try {
            ...
            // Channel closed before we could send — acceptable
        } catch (IOException _) {
