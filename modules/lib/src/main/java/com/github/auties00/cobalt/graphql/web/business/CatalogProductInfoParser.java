...
/**
 * Projects the rich catalog-product JSON node onto Cobalt's {@link BusinessProduct} model.
 *
 ...
 * projection so each response decoder maps a product node identically without duplicating the
...
public final class CatalogProductInfoParser {
    ...
    /**
     * Parses a single catalog-product JSON node into a {@link BusinessProduct}.
     *
     ...
     *
     * @param obj the catalog-product node, possibly {@code null}
    ...
    /**
     * Reads the hidden flag from a catalog-product node in either of its two encodings.
     *
     ...
     *
     * @param obj the catalog-product node, never {@code null}
    ...
    /**
     * Parses the image array of a catalog-product media node into {@link BusinessProductImage} values.
     *
    ...
    /**
     * Parses the video array of a catalog-product media node into {@link BusinessProductVideo} values.
     *
     * @implNote This implementation skips entries that lack the required id or either url, and entries
    ...
    /**
     * Parses the sale-price node of a catalog product into a sale-price block.
     *
     * @param obj the sale-price node, possibly {@code null}
    ...
    /**
     * Parses the compliance-info node of a catalog product into a compliance block.
     *
     ...
     *
     * @param obj the compliance-info node, possibly {@code null}
    ...
    /**
     * Parses the importer-address node of a compliance block into an importer-address model.
     *
     * @param obj the importer-address node, possibly {@code null}
