...
/**
 * Open-loop pitch-lag estimator of the MLow speech encoder, the port of {@code smpl_pitch} in
 * {@code smpl_pitch_util.c} together with its block-track table generation in {@code smpl_pitch.c} and the
 * rate-bias estimate of {@code smpl_encode_lags} in {@code smpl_param_coding.c}.
 *
 ...
 * perceptually weighted speech of the look-ahead-padded long-term-prediction buffer (the native
 * {@code ltp_buf}), not on the raw residual: the caller filters each frame through the pitch perceptual
 * weighting filter into that buffer before the call. The search is a coarse-to-fine normalized
 * cross-correlation peak pick:
 * <ul>
 ...
 *   <li><b>Stage-1 normalized correlation.</b> Per {@value #PITCH_NUM_SUBFRAMES}-way pitch subframe the
 *       lag energy {@code E1} (sliding-window, {@link #calcE1}), the cross-correlation {@code C} and the
 *       target energy {@code E2} are formed over the {@code [SMPL_MINPITCH_STAGE1, SMPL_MAXPITCH_STAGE1]}
 *       lag range. {@code E} is the geometric-style mean energy {@code (0.5 (sqrt E1 + sqrt E2))^2}.</li>
 *   <li><b>Upsample to the {@value #COARSE_FS_KHZ} kHz coarse grid.</b> {@code C} is band-limited
 *       interpolated and {@code E} is linearly interpolated back to full rate, yielding the coarse
 *       normalized correlation {@code H = C / E} over {@value #NUMLAGS_COARSE} lags.</li>
 ...
 *       recomputed at the input rate and fractionally upsampled by two, and each block segmentation's
 *       per-segment lag is the peak of the energy-weighted combined correlation.</li>
 ...
 * the conditional rate estimate). {@link #reset()} clears the lag-block carry exactly where the native encoder
 * resets {@code prev_lagblk}/{@code prev_lagidx}; an inactive-voice frame clears all carry. The
 * {@code numstates1} survivor count and the {@code lowRate} flag are fixed at construction from the encoder
 ...
 * float bit, so the native arithmetic is reproduced exactly. The correlation and energy dot products
 * ({@link #nrg}, {@link #dotProd}, {@link #dotProd20}, {@link #dotProd40}) accumulate with the two-wide paired
 * reduction the {@code -Ofast} build emits: {@code smpl_pitch_util.c} and {@code smpl_codec_util.c} are
 * compiled with fast-math and SSE2 (not AVX), so each reduction is an even-lane and an odd-lane running sum
 * added once at the end, not a left-to-right scan. A scalar sum rounds differently and, at the high
 * cancellation of these correlations, flips a survivor index. The {@link #getMaxi} and {@link #getMaxiK}
 * tournaments reproduce the native strict-greater-than comparison with the leaf-flag toggle and the
 * {@code -FLT_MAX} removal and re-propagation. The high-pass and resampling filters, the {@code calc_E1}
 * running window and the {@code smpl_encode_lags} bit estimate are bit-faithful ports.
 *
 * <p>Scope is the shipped SMPL 16 kHz / 60 ms / mono high-rate configuration: 20 ms frames, eight pitch
 * subframes, the {@value #NUM_BLOCKTRACKS} 20 ms block tracks and {@value #NUM_BLOCKSEGS} block segmentations
 * of {@link PitchTables}, and the high-quality search path ({@code low_complexity_mode == false}, the band
 * -limited correlation interpolation and the {@value #H_THRES_HQ} correlation gate). The decode-side lag
 * tables this estimator shares are owned by {@link PitchTables}; the encoder-only block-track spans and the
 * generated block tracks are built once here.
 *
 * @implNote This implementation keeps the native flat correlation buffers and reproduces the in-place upsample
 * aliasing of {@code smpl_pitch} verbatim: the coarse-rate {@code C}/{@code E} use the
 * {@code numlags_C}/{@code numlags_E} strides, and the fractional full-rate {@code C}/{@code E} overwrite them
 * from the high subframe down to the low one with the {@code (PITCH_NUM_BLOCKS * 2 * PITCHBLOCK + offset)}
 * stride, so the final-search reads land on exactly the native addresses. The {@code low_complexity_mode}
 * branch (survivor count at most four) is retained for completeness but never taken at the shipped complexity.
 * The {@code SMPL_PITCH_SHORTWGHT2} pitch-multiple bias is compiled out in the reference and is therefore
 * omitted; the {@code SMPL_PITCH_SPEC_HARM_BIAS} spectral harmonicity bias is enabled and ported in full.
 */
public final class OpenLoopPitch {
    /**
     * Pitch search sample rate in kHz, the native {@code SMPL_PITCH_FS_KHZ}.
     */
    private static final int FS_KHZ = 16;

    /**
     * Stage-1 (half-rate) pitch search sample rate in kHz, the native {@code SMPL_PITCH_STAGE_1_FS_KHZ}.
     */
    private static final int STAGE1_FS_KHZ = 8;

    /**
     * Coarse-grid pitch search sample rate in kHz, the native {@code SMPL_PITCH_COARSE_FS_KHZ}.
     */
    private static final int COARSE_FS_KHZ = 16;

    /**
     * Number of pitch subframes per 20 ms frame, the native {@code SMPL_PITCH_NUM_SUBFRAMES}.
     */
    private static final int PITCH_NUM_SUBFRAMES = 8;

    /**
     * Minimum pitch period in milliseconds, the native {@code SMPL_MINPITCH_MS}.
     */
    private static final int MINPITCH_MS = 2;

    /**
     * Maximum pitch period in milliseconds, the native {@code SMPL_MAXPITCH_MS}.
     */
    private static final int MAXPITCH_MS = 20;

    /**
     * Minimum pitch period in samples at the search rate, the native {@code SMPL_MINPITCH_LEN}.
     */
    private static final int MINPITCH_LEN = MINPITCH_MS * FS_KHZ;

    /**
     * Maximum pitch period in samples at the search rate, the native {@code SMPL_MAXPITCH_LEN}.
     */
    private static final int MAXPITCH_LEN = MAXPITCH_MS * FS_KHZ;

    /**
     * Total interpolation delay of the resampling chain, the native {@code SMPL_PITCH_TOT_INTERP_DELAY}.
     */
    private static final int TOT_INTERP_DELAY = 6;

    /**
     * Smallest stage-1 lag searched, the native {@code SMPL_MINPITCH_STAGE1}.
     */
    private static final int MINPITCH_STAGE1 = MINPITCH_MS * STAGE1_FS_KHZ - TOT_INTERP_DELAY;

    /**
     * Largest stage-1 lag searched, the native {@code SMPL_MAXPITCH_STAGE1}.
     */
    private static final int MAXPITCH_STAGE1 = MAXPITCH_MS * STAGE1_FS_KHZ + TOT_INTERP_DELAY;

    /**
     * Stage-1 lag subframe length in samples, the native {@code SMPL_PITCH_LAG_SUBFRLEN_STAGE1}.
     */
    private static final int LAG_SUBFRLEN_STAGE1 = STAGE1_FS_KHZ * 40 / FS_KHZ;

    /**
     * Input-rate lag subframe length in samples, the native {@code SMPL_LAG_SUBFRLEN}.
     */
    private static final int LAG_SUBFRLEN = 40;

    /**
     * Half-block length in samples at the search rate, the native {@code PITCHBLOCK}
     * ({@code SMPL_PITCHBLOCK_MS * SMPL_PITCH_FS_KHZ}).
     */
    private static final int PITCHBLOCK = 2 * FS_KHZ;

    /**
     * Full lag-block size in fractional lag indices, the native {@code BLOCKSIZE} ({@code 2 * PITCHBLOCK}).
     */
    private static final int BLOCKSIZE = 2 * PITCHBLOCK;

    /**
     * Number of coarse lag blocks, the native {@code PITCH_NUM_BLOCKS}.
     */
    private static final int PITCH_NUM_BLOCKS = (MAXPITCH_MS - MINPITCH_MS) / 2;

    /**
     * Number of lags on the coarse grid, the native {@code numlags_coarse} (and {@code SMPL_PITCH_NUMLAGS_COARSE}).
     */
    private static final int NUMLAGS_COARSE = COARSE_FS_KHZ * (MAXPITCH_MS - MINPITCH_MS);

    /**
     * Smallest coarse-grid lag, the native {@code minpitch_coarse}.
     */
    private static final int MINPITCH_COARSE = COARSE_FS_KHZ * MINPITCH_MS;

    /**
     * Coarse-grid half-block length, the native {@code pitchblock_coarse}.
     */
    private static final int PITCHBLOCK_COARSE = 2 * COARSE_FS_KHZ;

    /**
     * Number of stage-1 lags, the native {@code numlags} before any upsampling.
     */
    private static final int NUMLAGS_STAGE1 = MAXPITCH_STAGE1 - MINPITCH_STAGE1 + 1;

    /**
     * Number of input-rate lags, the native {@code NUMLAGS_FS}.
     */
    private static final int NUMLAGS_FS = FS_KHZ * (MAXPITCH_MS - MINPITCH_MS);

    /**
     * Number of candidate 20 ms block tracks, the native {@code NUM_BLOCKTRACKS}.
     */
    private static final int NUM_BLOCKTRACKS = 187;

    /**
     * Number of 20 ms block segmentations, the native {@code NUM_BLOCKSEGS}.
     */
    private static final int NUM_BLOCKSEGS = 217;

    /**
     * Half-rate decimation FIR delay, the native {@code PITCH_DOWNSAMP_DELAY}.
     */
    private static final int DOWNSAMP_DELAY = 7;

    /**
     * Band-limited correlation interpolation FIR half-length, the native {@code PITCH_INTERPOL_DELAY_C}.
     */
    private static final int INTERPOL_DELAY_C = 4;

    /**
     * Lag-jitter penalty weight, the native {@code SMPL_PITCH_DELTAWGHT}.
     */
    private static final float DELTAWGHT = 0.1439f;

    /**
     * Per-track short-lag bias weight, the native {@code SMPL_PITCH_SHORTWGHT1}.
     */
    private static final float SHORTWGHT1 = 0.04f;

    /**
     * Spectral harmonicity bias weight, the native {@code SMPL_PITCH_SPEC_HARM_BIAS}.
     */
    private static final float SPEC_HARM_BIAS = 2.5f;

    /**
     * Previous-lag continuity bonus weight, the native {@code SMPL_PITCH_PREVWGHT}.
     */
    private static final float PREVWGHT = 0.7981f;

    /**
     * Fractional span of the previous lag the continuity bonus covers, the native {@code SMPL_PITCH_PREVWGHT_SPAN}.
     */
    private static final float PREVWGHT_SPAN = 0.15f;

    /**
     * Low-rate entropy-coding rate-bias weight, the native {@code SMPL_PITCH_RATEWGHT_LR}.
     */
    private static final float RATEWGHT_LR = 0.028f;

    /**
     * High-rate entropy-coding rate-bias weight, the native {@code SMPL_PITCH_RATEWGHT_HR}.
     */
    private static final float RATEWGHT_HR = 0.022f;

    /**
     * Block-track delta penalty reduction factor, the native {@code REDUCTION_FACTOR}.
     */
    private static final float REDUCTION_FACTOR = 0.7f;

    /**
     * Per-fractional-index lag-jitter weight, the native {@code PITCH_DELTAWGHT} ({@code SMPL_PITCH_DELTAWGHT / BLOCKSIZE}).
     */
    private static final float PITCH_DELTAWGHT = DELTAWGHT / BLOCKSIZE;

    /**
     * Coarse-correlation gate for the high-quality fine search, the native {@code H_thres} when
     * {@code low_complexity_mode} is false.
     */
    private static final float H_THRES_HQ = 0.25f;

    /**
     * Spectral analysis half-spectrum length, the native {@code SMPL_F_LEN}.
     */
    private static final int F_LEN = 257;

    /**
     * Length of the spectral harmonicity result cache, the native {@code spectral_harm_cache} size.
     */
    private static final int HARM_CACHE_LEN = 50;

    /**
     * Sentinel for an unfilled spectral harmonicity cache slot, the native {@code HARMONICITY_UNDEF}.
     */
    private static final float HARMONICITY_UNDEF = -10000.0f;

    /**
     * Number of harmonic peak/valley measurements in the spectral harmonicity, the native {@code NUM_HARMS}.
     */
    private static final int NUM_HARMS = 4;

    /**
     * Seg-length cache key shift, the native {@code PITCH_CACHE_BITS_BLOCK}.
     */
    private static final int CACHE_BITS_BLOCK = 4;

    /**
     * Subframe cache key shift, the native {@code PITCH_CACHE_BITS_SEG_LEN}.
     */
    private static final int CACHE_BITS_SEG_LEN = 3;

    /**
     * Total span of the lag-index cache, the native {@code lagind_cache} length
     * ({@code 1 << (PITCH_CACHE_BITS_SF + PITCH_CACHE_BITS_SEG_LEN + PITCH_CACHE_BITS_BLOCK)}).
     */
    private static final int LAGIND_CACHE_LEN = 1 << (3 + CACHE_BITS_SEG_LEN + CACHE_BITS_BLOCK);

    /**
     * Half-rate decimation FIR, the native {@code pitch_downsamp_filt}.
     */
    ...
    /**
     * Band-limited correlation interpolation FIR, the native {@code pitch_interpol_filt_C}.
     */
    ...
    /**
     * High-pass moving-average numerator {@code {1, -1}}, the native {@code pitch_hp_b}.
     */
    private static final float[] HP_B = {1.0f, -1.0f};

    /**
     * High-pass auto-regressive denominator {@code {1, -0.96}}, the native {@code pitch_hp_a}.
     */
    private static final float[] HP_A = {1.0f, -0.96f};

    /**
     * Peak weights of the spectral harmonicity peak/valley folding, the native {@code mag_peak_weights}.
     */
    private static final float[] MAG_PEAK_WEIGHTS = {1.0f, 10.0f, 1.0f};

    /**
     * Valley weights of the spectral harmonicity peak/valley folding, the native {@code mag_valley_weights}.
     */
    private static final float[] MAG_VALLEY_WEIGHTS = {5.0f, 2.0f, 5.0f};

    /**
     * Block-track to block-segmentation span table, the native {@code smpl_pitch_blocksegs_ix} flattened to
     * interleaved {@code [offset, count]} pairs read as unsigned bytes.
     *
     * <p>Track {@code t} spans the block segmentations {@code [BLOCKSEGS_IX[2t], BLOCKSEGS_IX[2t] +
     * BLOCKSEGS_IX[2t + 1])}. The reference exposes this as {@code smpl_pitch_blocksegs_ix[t][0..1]}; this is
     * the encoder-only companion of the decode tables {@link PitchTables} already holds, embedded here so the
     * encode package needs no new accessor on the table layer.
     */
    ...
    /**
     * One reconstructed block track: the per-subframe lag block, the mean block index, and the cumulative
     * block jitter, the native {@code PITCH_blocktrack} produced by {@code smpl_gen_blocktrack}.
     *
    ...
    /**
     * The result of one open-loop pitch estimate, the native output parameters of {@code smpl_pitch}.
     *
     * @param lags         the fractional pitch lag for each subframe, the native {@code lags}
     * @param laginds      the integer fractional lag index for each subframe, the native {@code laginds}
     * @param pitchCorr    the best normalized pitch correlation, the native {@code pitchc}
     * @param blocksegIdx  the winning block-segmentation index, the native {@code blockseg_idx}
     * @param avgLag       the dominant-subframe average lag, the native {@code avg_lag}
     * @param harmStrength the spectral harmonicity of the average lag, the native {@code harm_strength}
     */
    ...
    /**
     * The shared 20 ms pitch decode data: block segmentations, the permutation, the rate-coding CMFs, and the
     * first-block ranges, the native {@code smpl_get_pitch_data(SMPL_PITCH_NUM_SUBFRAMES)}.
     */
    private final PitchTables.PitchData data;

    /**
     * The reconstructed 20 ms block tracks, the native {@code data20ms.blocktracks} built by
     * {@code smpl_gen_blocktrack}.
     */
    private final Blocktrack[] blocktracks;

    /**
     * The survivor count kept by the block-track tournament, the native {@code numstates1}.
     */
    private final int numstates1;

    /**
     * Whether the low-complexity search path is taken, the native {@code low_complexity_mode}
     * ({@code numstates1 <= 4}).
     */
    private final boolean lowComplexity;

    /**
     * Whether the low-rate entropy-coding rate-bias weight applies, the native {@code low_rate}.
     */
    private final boolean lowRate;

    /**
     * The previous frame's last fractional lag, the native {@code PitchEstimator.prev_lag}.
     */
    private float prevLag;

    /**
     * The previous frame's best pitch correlation, the native {@code PitchEstimator.prev_pitch_corr}.
     */
    private float prevPitchCorr;

    /**
     * The previous frame's last lag block, the native {@code PitchEstimator.prev_lagblk}; {@code -1} when
     * there is no previous frame in the packet or the carry was reset.
     */
    private int prevLagblk;

    /**
     * The previous frame's last lag index, the native {@code PitchEstimator.prev_lagidx}; {@code -1} when
     * there is no previous frame in the packet or the carry was reset.
     */
     ...
     * count, the native {@code smpl_create_pitch_estimator} then {@code smpl_update_pitch_params}.
     *
     ...
     *
     * @param numstates1 the survivor count kept by the block-track tournament, the native
     *                   {@code pitch_numstates1} (30 at the shipped highest complexity)
     * @param lowRate    whether the low-rate rate-bias weight applies, the native {@code lowRate}
     */
    ...
    /**
     * Clears the lag-block carry, the native {@code smpl_pitch_reset_cond} ({@code prev_lagblk = prev_lagidx =
     * -1}).
     *
     * <p>Must be called wherever the native encoder resets the lag carry: after an unvoiced frame and at the
     * last frame of a packet. The previous-lag continuity carry ({@code prevLag}, {@code prevPitchCorr}) is
     * not touched, matching the native reset which only clears the block/index carry.
     */
    public void reset() {
        this.prevLagblk = -1;
        this.prevLagidx = -1;
    ...
    /**
     * Estimates the open-loop pitch lags of one frame, the native {@code smpl_pitch}.
     *
     * <p>When {@code codedAsActiveVoice} is false the lags collapse to the minimum pitch, all indices are
     * zero, and the full carry ({@code prevLag}, {@code prevPitchCorr}, {@code prevLagblk}, {@code prevLagidx})
     * is cleared, matching the native inactive path. Otherwise the full coarse-to-fine search runs and the
     * carry is updated for the next frame. The {@code ltpBuf} holds the perceptually weighted speech of the
     * frame plus the look-ahead and resampling-delay padding (the native {@code ltp_buf} slice of length
     * {@code L}); {@code f2} is the frame's short-term power spectrum from the LPC analysis (the native
     * {@code lpcbuf_F2}, {@value #F_LEN} bins). The {@code ltpBuf} is not modified.
     *
     * @param ltpBuf             the look-ahead-padded weighted speech buffer, at least {@code L} samples
     * @param l                  the number of valid samples in {@code ltpBuf}, the native {@code L}
     * @param lookAhead          the number of look-ahead samples at the end of {@code ltpBuf}, the native
     *                           {@code lookAhead}
     * @param f2                 the frame short-term power spectrum, {@value #F_LEN} bins, the native
     *                           {@code F2}
     * @param codedAsActiveVoice whether the frame is coded as active voice, the native
     *                           {@code coded_as_active_voice}
     * @param numsubfrs          the number of pitch subframes, the native {@code numsubfrs} (always
     *                           {@value #PITCH_NUM_SUBFRAMES} for the shipped configuration)
    ...
    /**
     * Runs the full active-voice coarse-to-fine pitch search, the body of {@code smpl_pitch} after the
     * inactive guard.
     *
    ...
    private Result search(float[] ltpBuf, int l, int lookAhead, float[] f2, int numsubfrs,
        ...
        float[] e2Arr = new float[PITCH_NUM_SUBFRAMES];
        ...
        float[] sqrtE1 = new float[16 * (MAXPITCH_MS - MINPITCH_MS)];
        ...
        float[] sfWght = new float[PITCH_NUM_SUBFRAMES];
        calcSfWeights(e2Arr, numsubfrs, sfWght);

        float[] utils = new float[NUM_BLOCKTRACKS];
        ...
        float[] sqrtE1Blk = new float[2 * 16 + 1];
        for (int sf = 0; sf < numsubfrs; sf++) {
        ...
        float[] hComb = new float[2 * PITCHBLOCK];
        int nlaginds = 0;
        int[] blocksegsIx = new int[NUM_BLOCKSEGS];
        int[][] survLaginds = new int[NUM_BLOCKSEGS][PITCH_NUM_SUBFRAMES];
        int[] lagindCache = new int[LAGIND_CACHE_LEN];
    ...
    /**
     * Generates the block tracks from the block segmentations, the native {@code smpl_gen_blocktrack}.
     *
     ...
     *
     * @param blocksegs the reconstructed block segmentations indexed by C block-segmentation index
    ...
    /**
     * Decimates the high-passed buffer by two through the symmetric FIR, the native
     * {@code smpl_pitch_downsample}.
     *
     * <p>The center tap multiplies the delayed sample and the symmetric pairs fold around it. The first
     * {@value #DOWNSAMP_DELAY} stage-1 output samples are left zero (the caller zeroes them before the
     * high-pass writes its output past the delay); this writes the decimated samples in place into the same
     * buffer past the delay region.
     *
     * @param buf the buffer holding the delayed high-passed samples, decimated in place from the start
     * @param len the number of valid input samples, the native {@code L + offset}
     * @return the number of decimated output samples, the native return value
     */
    ...
    /**
     * Computes the sliding-window lag energy for one lag span, the native {@code smpl_calc_E1}.
     *
     * <p>The first entry is the energy of the {@code lag_subfrlen}-sample window at the largest lag; each
     ...
     *
     * @implNote This implementation groups the running update as {@code E1[i-1] + (add - sub)}, the
     * leading-minus-trailing squared difference added to the carry in one parenthesized term, matching the
     * {@code -Ofast} build that subtracts the two squares before accumulating; the left-to-right
     * {@code (E1[i-1] + add) - sub} rounds differently and shifts the input-rate lag energy that feeds the
     * normalized pitch correlation.
     *
     * @param e1          the destination energy vector
     * @param ltpbuf      the buffer to read the windows from
     * @param t           the target offset, the native {@code t}
     * @param minpitch    the smallest lag, the native {@code minpitch}
     * @param maxpitch    the largest lag, the native {@code maxpitch}
     * @param lagSubfrlen the energy window length, the native {@code lag_subfrlen}
     */
    ...
    /**
     * Computes the per-subframe lag energy, the native {@code smpl_pitch_calc_E1}.
     *
     * <p>One extended {@code E1} is computed over the lag span widened by the subframe count, then sliced per
     * subframe with the slice offset decremented by {@code lag_subfrlen} each subframe.
     *
     * @param e1          the destination per-subframe energy vector, {@code numsubfrs * numlags} entries
     * @param ltpbuf      the buffer to read the windows from
     * @param ltpbufLen   the number of valid samples in {@code ltpbuf}, the native {@code ltpbuf_len}
     * @param numsubfrs   the number of pitch subframes
    ...
    /**
     * Computes the per-subframe stage-1 cross-correlation and target energy, the native
     * {@code smpl_pitch_calc_C_E2}.
     *
     * <p>Per subframe the target window is correlated against every lagged window over the stage-1 lag span,
     * and the target energy is the window's self-correlation, floored at {@code 1e-9}.
     *
     * @param c         the destination cross-correlation, {@code numsubfrs * NUM_LAGS_STAGE1} entries
    ...
    /**
     * Linearly upsamples the energy buffer by two in place, the native {@code smpl_upsamp_E_fast}.
     *
     * <p>Each subframe's energy slice is doubled by inserting the average of adjacent samples, written from
     * the high subframe down to avoid overwriting the next subframe's input. The minimum lag doubles and the
     * lag count becomes {@code (n - 1) * 2}.
     *
     * @param numsubfrs the number of pitch subframes
    ...
    /**
     * Band-limited upsamples the correlation buffer by two in place, the native {@code smpl_upsamp_C_fast}.
     *
     * <p>Each subframe's correlation slice is doubled by the {@value #INTERPOL_DELAY_C}-tap symmetric
     * interpolation FIR, written from the high subframe down. The minimum lag doubles and the lag count
     * becomes {@code (n - PITCH_INTERPOL_DELAY_C) * 2}.
     *
     * @param numsubfrs the number of pitch subframes
     * @param minpitch  the current smallest lag
     * @param numlags   the current lag count
    ...
    /**
     * Linearly interpolates a slice by two, the native {@code upsamp_E_core}.
     *
     * <p>Writes backward from {@code yOff}: the interpolated sample is the average of two adjacent inputs, the
     * copied sample is the input. The {@code x} pointer walks backward, matching the native decrementing
     * pointers.
     *
    ...
    /**
     * Band-limited interpolates a slice by two, the native {@code upsamp_C_core}.
     *
    ...
    /**
     * Computes the subframe weights from the target energies, the native {@code calc_sf_weights}.
     *
    ...
    /**
     * Computes the cumulative absolute lag-index jitter across a survivor, the native {@code sumdeltas}.
     *
    ...
    /**
     * Computes the previous-lag continuity bonus, the native {@code get_prev_lag_bias}.
     *
    ...
    /**
     * Estimates the bit cost of coding a survivor's lags, the native {@code smpl_encode_lags} with a null
     * range coder and {@code mode == 1}.
     *
     ...
     * symbols. Each non-uniform symbol contributes {@code -log2((fh - fl) / ft)} of its windowed cumulative
     * mass function; the uniform first lag contributes a flat six bits.
     *
     * @param blocksegsIx the survivor's C block-segmentation index
     ...
     * @return the estimated bit cost, the native {@code n_bits}
     */
    ...
    /**
     * Estimates one symbol's bit cost, the native {@code ec_encode_wrap} with a null encoder context.
     *
     * <p>Returns {@code -log2((fh - fl) / ft)}, the ideal code length of a symbol whose cumulative mass
     * function window is {@code [fl, fh)} of total {@code ft}.
     *
    ...
    /**
     * Computes the spectral harmonicity at a pitch lag, the native {@code spectral_harmonicity}.
     *
     * <p>Measures the ratio of harmonic peaks to valleys at the low frequencies of the weighted power
     * spectrum {@code F2w}. The result is cached per quantized harmonic index; the cache is reset when
     * {@code resetCache} is set. For pitch periods above roughly 60 Hz the peaks and valleys are gathered with
     * parabolic weighting windows and combined into a weighted mean log peak/valley ratio; below that the
     * harmonicity is a flat {@code 0.1}.
     *
     * @implNote This implementation reproduces the four float reassociations the {@code -Ofast} build performs
     * on this routine, each of which shifts the reported {@code harm_strength} (voicing strength) by up to tens
     * of units in the last place where the log peak/valley ratio approaches zero:
     * <ul>
     *   <li>The reciprocal {@code 1 / harm_width} is strength-reduced to a single multiply
     *       {@code avg_lag * ((1 / inv_F2_step_Hz) / 16000)}, since {@code harm_width = (16000 / avg_lag) *
     *       inv_F2_step_Hz}; the precomputed constant rounds differently from a runtime {@code divss}.</li>
     ...
     *   <li>The per-harmonic log peak/valley ratio uses the verbatim {@link #logf}, not a rounded
     *       double-precision logarithm.</li>
     * </ul>
     * The {@code peak_valley_nrg} numerator and denominator and the final {@code harm_strength} dot product and
     * sum use the same {@code -Ofast} packed-SSE reductions as the pitch correlations ({@link #dotProd},
     * {@link #sumVec}).
     *
    ...
    /**
     * Computes the energy of a window, the native {@code smpl_nrg} with the {@code -Ofast} SSE2 reduction.
     *
     * <p>{@code smpl_codec_util.c} is compiled with fast-math and SSE2, so the sum of squares vectorizes into
     * a four-lane packed-single accumulator over four consecutive samples per step, reduced horizontally as
     * {@code (lane0 + lane2) + (lane1 + lane3)} and finished with a left-to-right scalar tail for the
     * remaining {@code n & 3} samples. This is reproduced exactly by {@link #dotProd4Wide}; a plain scalar sum
     * rounds differently and flips a survivor through the correlation tournament.
     *
     * @param x    the buffer
    ...
    /**
     * Applies the pitch pre-emphasis high-pass {@code smpl_filt_arma1} compiled at {@code -Ofast}, the
     * {@link #HP_B}{@code /}{@link #HP_A} pole-zero filter at the head of {@link #estimate}.
     *
     * <p>Runs the monic moving-average difference {@code y[i] = x[i] - x[i - 1]} (exact, reused from the
     * shared {@link Filters#ma1}) into the recursive {@link #hpAr1} pole. This is a dedicated encoder copy of
     * {@code smpl_filt_arma1} rather than the shared {@link Filters#arma1} because {@code smpl_filt.c} is
     * compiled with fast-math, and the divergent fast-math reassociation lives in the auto-regressive pole;
     * {@link Filters#ar1} keeps the literal source order for the decoder, so the pitch path needs its own
     * fast-math-faithful pole. The moving-average numerator has only two taps per output and reassociates to
     * itself, so it is shared unchanged.
     *
    ...
    /**
     * Applies the first-order auto-regressive pole of the pitch high-pass with the fast-math reassociation the
     * {@code -Ofast} build of {@code smpl_filt_ar1} emits.
     *
     * <p>Computes the leaky recursion {@code y[k] = x[k] + ar1 * y[k - 1]} over {@code n} samples, threading the
     * previous output through {@code state[stateOff]}. The native body unrolls five outputs at a time and
     * expands each in terms of the five input taps and the carried output, then fast-math reassociates each
     * expansion. Recovered byte-exact from the {@code -Ofast} reference object, the per-output add trees are:
     * <ul>
     ...
     * multiplication, and the trailing {@code n % 5} samples run the plain scalar recursion. The shared
     * {@link Filters#ar1} writes these expansions in literal source order, which differs in the last float bit;
     * the pitch correlation built from this filtered signal drives the survivor tournament, so the divergence
     * propagates to {@code pitchc} and on to the voiced decision and bitrate allocation.
     *
     * @apiNote This is a {@link OpenLoopPitch}-private fast-math copy of {@link Filters#ar1}; the shared filter
     * must keep its literal source order for the decoder, which is energy-matched rather than byte-identical.
     * @implNote This implementation reproduces the packed-scalar reassociation GCC's tree-reassociation pass
     * emits at {@code -O3 -msse -msse2 -Ofast} for the five-wide unrolled body; the grouping was recovered by an
     * exhaustive parenthesization search validated byte-exact against the reference object over thousands of
     * random windows.
     *
     * @param x        the input buffer
    ...
    /**
     * Computes a variable-length inner product, the native {@code smpl_dot_prod} with the {@code -Ofast} SSE2
     * reduction.
     *
     * <p>Reproduces the four-lane packed-single accumulation and the {@code (lane0 + lane2) + (lane1 + lane3)}
     * horizontal reduction the fast-math build emits for {@code smpl_dot_prod}; see {@link #dotProd4Wide}.
     *
    ...
    /**
     * Computes an inner product with the four-lane packed-single SSE2 reduction, the native {@code -Ofast}
     * vectorization of {@code smpl_dot_prod}, {@code smpl_nrg}, {@code dot_prod_20} and {@code dot_prod_40}.
     *
     * <p>Four consecutive products accumulate into four lanes, then the lanes reduce horizontally as
     * {@code (lane0 + lane2) + (lane1 + lane3)} (the native {@code movhlps}/{@code addps} then
     * {@code shufps}/{@code addps} sequence), and the remaining {@code n & 3} products are added left to right
     * with scalar {@code addss}. The {@code 20}-tap and {@code 40}-tap pitch correlations have no tail; the
     * harmonicity correlation does.
     *
     * @param a    the first buffer
     * @param aOff the offset into the first buffer
     * @param b    the second buffer
    ...
    /**
     * Computes a 20-tap inner product with the fully-unrolled SSE2 reduction tree, the native
     * {@code dot_prod_20}.
     *
     * <p>The fixed-length {@code dot_prod_20} unrolls to five four-lane products {@code p0 .. p4} (the last
     * being the trailing four taps) reduced as {@code ((p0 + p1) + p4) + (p2 + p3)} into one four-lane
     * accumulator, then {@code (lane0 + lane2) + (lane1 + lane3)} horizontally. This add tree is the exact
     * {@code -Ofast} SSE2 schedule reconstructed from the reference object and differs from the running
     * {@link #dotProd4Wide} tree; the difference is one float bit and is load-bearing because the stage-1
     * correlation it feeds drives the survivor tournament.
     *
     * @param a    the first buffer
     * @param aOff the offset into the first buffer
     * @param b    the second buffer
    ...
    /**
     * Computes a 40-tap inner product with the ten-group packed-single reduction the inlined native
     * {@code dot_prod_40} emits, the input-rate pitch correlation.
     *
     * <p>Inside {@code smpl_pitch} the {@code -Ofast} build fully unrolls {@code dot_prod_40} into ten
     * four-lane products {@code p0 .. p9} (group {@code g} multiplies taps {@code 4g .. 4g + 3} of the two
     * buffers with {@code mulps}) and reduces them lane-wise with the fixed unbalanced add tree
     * {@code (((p3 + p4) + p7) + ((p5 + p6) + p9)) + (((p1 + p2) + p8) + p0)}, then collapses the four lanes
     * with the {@code movhlps}/{@code addps} then {@code shufps}/{@code addps} sequence to
     * {@code (lane0 + lane2) + (lane1 + lane3)}. This add tree is the exact instruction schedule reconstructed
     * from the reference object, and it differs from both {@link #dotProd4Wide} (the running four-lane
     * {@code smpl_dot_prod}) and a plain scalar sum; its last float bit feeds the {@code C}/{@code E} the final
     * search reads, so the reported {@code pitchc} matches the native value bit for bit.
     *
     * @param a    the first buffer
     * @param aOff the offset into the first buffer
     * @param b    the second buffer
    ...
    /**
     * Computes a 40-tap self-energy with the ten-group packed-single reduction the inlined native
     * {@code dot_prod_40(x, x)} emits, the input-rate target energy {@code E2}.
     *
     * <p>The self-energy call site of {@code dot_prod_40} (the native {@code E2[sf] = dot_prod_40(ltp_buf_ptr,
     * ltp_buf_ptr)}) inlines to a different instruction schedule than the cross-correlation call site
     * {@link #dotProd40} even though the source is the same function: the {@code -Ofast} build squares each
     * four-tap group with one {@code mulps} into products {@code p0 .. p9} and reduces them lane-wise with the
     * add tree {@code (((p0 + p1) + p6) + p9) + (((p2 + p3) + p7) + ((p4 + p5) + p8))}, then collapses the four
     * lanes with the {@code movhlps}/{@code addps} then {@code shufps}/{@code addps} sequence to
    ...
    /**
     * Multiplies four consecutive sample pairs into a four-lane vector, the native {@code mulps}.
     *
     * @param a    the first buffer
    ...
    /**
     * Adds two four-lane vectors, the native {@code addps}.
     *
    ...
    /**
     * Sums a window with the four-lane packed-single SSE2 reduction, the native {@code smpl_sum_vec} under the
     * {@code -Ofast} build.
     *
     * <p>{@code smpl_codec_util.c} is compiled with fast-math and SSE2, so {@code smpl_sum_vec} does not stay a
     * left-to-right scan: the first element seeds a scalar accumulator, the remaining {@code len - 1} elements
     * accumulate into four packed lanes ({@code lane_j} running over element indices {@code 1 + 4c + j}), the
     * lanes reduce horizontally as {@code (lane0 + lane2) + (lane1 + lane3)} (the {@code movhlps}/{@code addps}
     * then {@code shufps}/{@code addps} sequence), that reduction is added to the seed, and the trailing
     * {@code (len - 1) & 3} elements are added left to right with scalar {@code addss}. This is the exact
     * schedule the reference object emits; a plain left-to-right sum rounds differently from {@code len == 13}
     * up and shifts the {@code spectral_harmonicity} peak/valley normalization, which drives the
     * {@code harm_strength} (voicing strength) the final pitch search reports.
     *
     * @param x   the buffer
    ...
    /**
     * Takes the element-wise square root of a window in place, the native {@code smpl_sqrt_vec}.
     *
     * @param x   the buffer
     * @param len the window length
     */
    ...
    /**
     * Returns the maximum of a window, the native {@code smpl_maximum}.
     *
     * @param x   the buffer
     * @param off the offset of the first sample
     * @param len the window length
    ...
    /**
     * Returns the index of the maximum of a window through the pairwise tournament, the native
     * {@code smpl_get_maxi}.
     *
     ...
     * comparison is strict greater-than so ties keep the lower index, matching the native float tie-break.
     *
    ...
    /**
     * Returns the indices of the {@code k} highest values through the leaf-flag tournament, the native
     * {@code smpl_get_maxi_K}.
     *
     * <p>The values are folded in halves once; each of the {@code k} extractions finds the current winning
     * leaf, recovers its index, records the leaf-flag toggle so a leaf's two children are reported in order,
     * then removes the winner (by writing {@code -FLT_MAX}) and re-propagates the affected path up the
     * tournament. The strict greater-than comparison keeps the native float tie-break.
     *
     * @param x    the values
    ...
    /**
     * Reciprocal table for the {@code logf} argument reduction, the native {@code logf} first lookup table.
     *
    ...
    /**
     * High part of the {@code logf} reduced-interval logarithm, the native {@code logf} second lookup table.
     *
    ...
    /**
     * Low part of the {@code logf} reduced-interval logarithm, the native {@code logf} third lookup table.
     *
     * <p>Entry {@code idx} is the {@code float} bit pattern of the low part of the logarithm of the reduction
     * point of {@link #logf}, recovered with {@link Float#intBitsToFloat(int)} and accumulated with the low
     * part of {@code e * ln2}. The bit patterns are reproduced verbatim from the platform runtime.
     */
    ...
    /**
     * Computes a single-precision natural logarithm, the native {@code logf} linked into the spectral
     * harmonicity peak/valley ratio.
     *
     * <p>This is a verbatim port of the platform {@code logf}, whose result is the per-harmonic log
     * peak/valley ratio of {@link #spectralHarmonicity}; the weighted mean of those logs is the reported
     * {@code harm_strength} (voicing strength), so the routine must match bit for bit. A rounded
     * double-precision logarithm differs by up to one unit in the last place, and near a near-unity peak/valley
     * ratio (where {@code logf} approaches zero) that single-bit deviation is amplified into the tens-of-bits
     * relative error observed on the reported harmonicity. The routine splits {@code x} into a mantissa-table
     * index from the top mantissa bits (with the rounding bit folded in) and an exponent {@code e}; the reduced
     * argument {@code r = recip[idx] * (idx_value - mantissa)} drives the cubic {@code r + r^2 (r/3 + 1/2)}, and
     * the result reconstructs {@code log(x)} from the two split logarithm tables {@link #LOGF_HI} and
     * {@link #LOGF_LO} and the high and low parts of {@code e * ln2}. Inputs within {@code 1/16} of one take a
     * dedicated {@code atanh}-style series in {@code s = (x - 1) / (x + 1)}.
     *
    ...
    /**
     * Computes the base-2 logarithm in single precision, the native {@code log2f}.
     *
     * @param x the operand
    ...
    /**
     * Rounds to the nearest integer, ties away from zero, in single precision, the native {@code roundf}.
     *
     * @param x the operand
