...
public record CallParticipantJid(Jid userJid, List<CallDeviceJid> devices, PrivacyToken privacyToken, byte[] botOptions) {
    ...
    /**
     * Compares this participant with another object for value equality.
     *
     * <p>Two participants are equal when they carry the same user JID, the same normalized
     * device list, an equal privacy token, and byte identical bot options. This override
     * replaces the record's default equality so the opaque bot options array is compared by
     * content via {@link Arrays#equals(byte[], byte[])} rather than by reference.
     *
     * @param obj the object to compare against
     * @return {@code true} if {@code obj} is a {@code CallParticipantJid} with equal
     ...
     *
     * <p>The hash combines the user JID, the device list, the privacy token, and the
     * content hash of the bot options via {@link Arrays#hashCode(byte[])}, so that
     * participants equal by content share a hash code.
     *
     * @return the hash code for this participant
     */
    @Override
    public int hashCode() {
    ...
    /**
     * Returns a diagnostic string describing this participant.
     *
     * <p>The rendering lists the user JID and device list in full, and reports the privacy
     * token and bot options by length only ({@code -1} when absent) so the opaque payload
     * bytes are not spilled into logs.
     *
     * @return a human readable representation of this participant
     */
    @Override
