 ...
 * over-use detector trendline over their inter-arrival delay, and drives an AIMD rate control from the
 ...
 * {@link AimdRateControl}. The rate control decreases multiplicatively on over-use and increases
 ...
 * thread.
 *
 * @implNote This implementation ports the GoogCC delay-based pipeline (InterArrival -> TrendlineEstimator
 * -> AimdRateControl) reached in the wa-voip engine through {@code bwe/bwe_webrtc_delay_based.cc}
 * (fn6193, fn6197 {@code process_rtp}, fn6199 {@code get_estimate}). The captured {@code voip_settings}
 * blob (re/calls2-spec/captures) confirms {@code delay_based_bwe_bitrate_estimator_enabled=1} and
 * {@code delay_based_bwe_use_pp=1}; the inter-arrival burst window and the acknowledged-throughput
 * window length are the upstream WebRTC defaults, since the exact values are not carried in the
 * captured blob (re/calls2-spec/SPEC.md sec 15.1).
 */
public final class GccDelayBasedEstimator implements BandwidthEstimator {
    ...
    /**
     * Default initial delay-based estimate, in bits per second, before any over-use feedback.
     */
    ...
    /**
     * The over-use detector trendline driven by inter-group delay deltas.
     */
    ...
    // TODO: the 0x800/0x4000 congestion-timing bits in get_congestion_signals (fn4226,
    //  rate_control/wa_rate_control.cc) compare their thresholds against min(stat+0xa0, +0x128) (remote
    //  timing) and min(stat+0x130, +0x128) (local timing), where stat is the rate-control stream-stat
    //  struct built by get_update_rc_data_internal (fn4199, wa_rate_control_state_updater.cc), NOT this
    //  GoogCC delay-based estimator. The min-over-window semantics of +0xa0/+0x128/+0x130 live in fn4199
    //  and are not modeled by this estimator; interArrivalTimingMs() / oneWayDelayVariationMs() expose the
    //  inter-group inter-arrival timing this delay-based pipeline genuinely tracks (the closest faithful
    //  source), but the exact stat-struct min windows the bits compare are unrecovered here and need
    //  fn4199's stat accounting threaded in before CongestionSignalDetector.evaluate can act on the timing
    //  bits. SPEC 15.4.
