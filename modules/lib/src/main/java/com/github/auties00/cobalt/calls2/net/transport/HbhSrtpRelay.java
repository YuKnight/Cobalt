 ...
 * DATA over the call's one DTLS-wrapped SCTP data channel; the DTLS layer protects the SCTP transport, while
 * the media payload stays hop-by-hop SRTP rather than DTLS-SRTP
 * (re/calls2-spec/captures/webrtc-datachannel-transport-2026-06-21.md). This is distinct from the
 ...
 * @implNote This implementation surface reproduces {@code wa_hbh_srtp_relay.cc}: the SRTP context
 *           created by {@code create_srtp_context_for_hbh_srtp} keyed by {@code derive_hbh_srtp_key}
 *           (fn4808) with the suite from {@code fill_hbh_srtp_crypto} (fn4809), the per-stream
 *           authenticated-feedback info of {@code hbh_srtp_create_srtp_afb_info} (fn4821), and the
 *           RTCP-feedback subscription table of {@code hbh_srtp_fb_subscribe} (fn4814).
 */
public interface HbhSrtpRelay extends AutoCloseable {
     ...
     *           leave the first {@code length} bytes' header intact (SRTP encrypts only the payload).
     */
