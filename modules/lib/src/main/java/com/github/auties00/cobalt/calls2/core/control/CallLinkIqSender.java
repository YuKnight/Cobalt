 ...
 * {@link CallMessage} and returns the reply {@link Node} the controller parses into the operation's ack.
 ...
 * reply node. It MAY throw to signal a transport failure or a server error reply; a controller treats a
...
public interface CallLinkIqSender {
     ...
     *
     * @implSpec An implementation MUST block until the reply arrives and return the reply node, or throw on
     * a transport or server failure.
     * @param request the typed request to send; never {@code null}
     * @return the reply node the caller parses into the operation's ack; never {@code null}
     */
    Node sendForReply(CallMessage request);
}
