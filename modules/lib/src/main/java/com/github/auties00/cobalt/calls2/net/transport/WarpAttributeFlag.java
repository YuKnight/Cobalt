 ...
 * message carries.
 *
 * <p>A {@link WarpMessage} is a five-byte fixed header followed by a flag-selected sequence of
 * attributes. The flags live in one mandatory byte at offset four (carrying bits {@code 0..7}) and one
 * optional extension byte at offset five (carrying bits {@code 8..15}, present only when
 * {@link #EXT_FLAG} is set); offsets zero through three carry the type byte, the packed length byte, and
 * the big-endian timestamp {@code u16}. Each constant names one bit, records its numeric mask, and
 * records whether it lives in the base byte or the extension byte. The serializer walks the bits in
 * ascending order and appends each present attribute's payload in that order, so the ordinal of these
 * constants is also the wire append order.
 *
 ...
 * extension-byte attribute and is legal only on a standalone (not piggybacked) WARP message; setting
 * it implies {@link #EXT_FLAG}.
 *
 * @implNote This implementation reproduces the attribute-flag switch of {@code wa_serialize_warp_message_s}
 *           (fn5153) cross-referenced with {@code set_send_attr_options} (fn5160) and
 *           {@code warp_mcs_prepare_warp_pr} (fn4043) from the wa-voip WASM module {@code ff-tScznZ8P}
 *           ({@code sfu/wa_warp_msg.cc}, {@code transport/wa_transport_warp.cc}). Bits {@code 6} and
 *           {@code 7} of the base byte are logged then skipped by the shipped serializer and so are not
 *           modeled here; the value bits modeled are {@code 0x01}, {@code 0x02}, {@code 0x04},
 *           {@code 0x08}/{@code 0x20}, {@code 0x40}, the {@code 0x80} extension marker, and the
 *           {@code 0x100} extension attribute.
 */
public enum WarpAttributeFlag {
     ...
     *
     * <p>It reports the device's current downlink bandwidth estimate (the engine's {@code rc->dl_bw}
     * divided by one thousand), attached when a recent non-zero sample exists.
     */
    ...
    /**
     * The participant-report attribute, a ten-byte rate-control report block.
     *
     * <p>It is always paired with {@link #PARTICIPANT_REPORT_COMPANION} so the two appear together as
     * the combined mask {@code 0x28}; the block is the {@link WarpParticipantReport} struct.
     */
     ...
     * {@code u32}.
     *
     * <p>It carries the four-byte value the transport SRTP layer produces for the current RTP index,
     * used by the SFU to authenticate per-stream feedback.
     */
