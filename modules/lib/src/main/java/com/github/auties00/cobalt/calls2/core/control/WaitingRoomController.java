...
import com.github.auties00.cobalt.calls2.signaling.WaitingRoomAdmitAck;
import com.github.auties00.cobalt.calls2.signaling.WaitingRoomAdmitStanza;
import com.github.auties00.cobalt.calls2.signaling.WaitingRoomDenyAck;
import com.github.auties00.cobalt.calls2.signaling.WaitingRoomDenyStanza;
import com.github.auties00.cobalt.calls2.signaling.WaitingRoomLeaveAck;
import com.github.auties00.cobalt.calls2.signaling.WaitingRoomLeaveStanza;
import com.github.auties00.cobalt.calls2.signaling.WaitingRoomToggleAck;
import com.github.auties00.cobalt.calls2.signaling.WaitingRoomToggleStanza;
 ...
 *
 * @implNote This implementation reproduces the waiting-room control of the wa-voip WASM module
 * {@code ff-tScznZ8P} ({@code waiting_room.cc}): {@code call_waiting_room_admit_internal} (fn11138) sends
 * IQ type {@code 0x47} (message type {@code 71}, empty list admits all) and {@code deny} sends IQ type
 * {@code 0x49} (type {@code 73}); the toggle is type {@code 69}; the local user's lobby withdrawal sends
 * the {@code waiting_room_leave} (message type {@code 67}), answered by the type-{@code 68} leave receipt.
 * The host learns the lobby occupants from
 * the inbound {@code <waiting_room>} update and emits event {@code 0x9e}
 * ({@code WaitingRoomStateChanged}); the admit and deny acks emit events {@code 0xa5} and {@code 0xa6}, the
 * toggle ack event {@code 0xa1}; the local user's lobby progress emits event {@code 0x8b}
 * ({@code CallLinkLobbySelfStateChanged}) and a denial emits event {@code 0x9d} ({@code WaitingRoomDenied}).
 * The {@code WaitingRoomUserState} values ({@code outgoing}/{@code receipt}/{@code terminated}/{@code joined})
 * are modeled by {@link WaitingRoomUserState}.
 */
