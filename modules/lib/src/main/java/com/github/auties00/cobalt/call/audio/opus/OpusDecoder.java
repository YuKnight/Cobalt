...
public final class OpusDecoder implements AutoCloseable {
    ...
    public OpusDecoder(int sampleRate, int channels) {
        ...
        try {
            ...
            try {
                ...
                throw new OpusCodecException("opus_decoder_create failed", t);
            }
            int err = errSeg.get(ValueLayout.JAVA_INT, 0);
            if (err != Opus.OPUS_OK() || state.equals(MemorySegment.NULL)) {
                throw OpusCodecException.fromErr("opus_decoder_create", err);
            }
     ...
     *         interleaved PCM
     * @throws OpusCodecException if decoding fails
     */
    public short[] decode(byte[] packet, int frameSize) {
        ...
        try (var scratch = Arena.ofConfined()) {
            ...
            try {
                ...
                throw new OpusCodecException("opus_decode failed", t);
            }
            if (samples < 0) {
                throw OpusCodecException.fromErr("opus_decode", samples);
            }
     ...
     * @throws OpusCodecException if PLC fails
     */
    public short[] decodePacketLoss(int frameSize) {
        ...
        try {
            ...
            throw new OpusCodecException("opus_decode (PLC) failed", t);
        }
        if (samples < 0) {
            throw OpusCodecException.fromErr("opus_decode (PLC)", samples);
        }
        var out = new short[samples * channels];
        MemorySegment.copy(pcmBuf, ValueLayout.JAVA_SHORT, 0, out, 0, out.length);
        return out;
    ...
    public void resetState() {
        ...
        try {
            ...
            throw new OpusCodecException("opus_decoder_ctl OPUS_RESET_STATE failed", t);
        }
        if (rc != Opus.OPUS_OK()) {
            throw OpusCodecException.fromErr("opus_decoder_ctl OPUS_RESET_STATE", rc);
        }
    }
