package com.github.auties00.cobalt.graphql.web;
 ...
 *
 * <p>This hierarchy mirrors the role {@code MexOperation} plays for the MEX transport: it collapses
 ...
 * (url-encoded form versus stanza node), and authentication (the WhatsApp Web session cookie versus
 * the already-authenticated socket session). The WhatsApp Web GraphQL transport is not stanza-based, so it lives
 * outside the {@code node} package alongside the Facebook GraphQL transport rather than with the socket-carried
 * operation families.
 *
