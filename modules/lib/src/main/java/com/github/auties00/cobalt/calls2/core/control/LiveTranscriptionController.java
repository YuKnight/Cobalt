 ...
 * language, and the sequence id the host uses to correlate and order fragments of an utterance. It is a
 * thin bridge from the application-data layer to the event bus; it owns no state and no timers, and it is
 * bound to its event sink at construction.
 *
 ...
 * binds, pairing the inbound fragment with the device JID it arrived from rather than a sender-less
 * fragment stream.
 *
 * @implNote This implementation reproduces the transcript surfacing of the wa-voip WASM module
 * {@code ff-tScznZ8P}: an inbound {@code liveTranscriptionInfo} message on the application-data stream is
 * emitted as event {@code 0x9b} ({@code TranscriptReceived}) with the recovered payload
 * {@code {jid, text, lang, seq}}. The speaking participant is supplied by the application-data layer's
 * sender attribution rather than by the {@link LiveTranscriptionInfo} body; a fragment with no caption is
 * dropped because there is nothing to surface.
 */
public final class LiveTranscriptionController {
    /**
     ...
     */
    public LiveTranscriptionController(CallEventSink events) {
     ...
     *
     * @param sender   the device JID of the speaking participant, supplied by the application-data layer's
     *                 sender attribution; never {@code null}
