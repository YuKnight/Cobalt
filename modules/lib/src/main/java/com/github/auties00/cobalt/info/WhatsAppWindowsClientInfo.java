...
final class WhatsAppWindowsClientInfo implements WhatsAppClientInfo {
     ...
     * fails.
     *
     * @implNote The fallback is chosen to satisfy the
     *           {@code WAWebClientPayload.C} length check (six digits so the
     *           {@code mcc}/{@code mnc} override fires) and the feature gate
     *           thresholds enforced elsewhere in the Windows hybrid bundle.
     *           {@code WAWebHybridUtils} expects {@code >=256700},
     *           {@code WAWebWindowsConstants} caps some behaviour at
     *           {@code <257700}, and {@code WAWebWindowsBuildType} branches
     *           on {@code >260000}. The concrete value is captured from a
     *           live production handshake so the offline fallback is
     *           indistinguishable from a genuine recent build.
     */
