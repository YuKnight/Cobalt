...
/**
 * Resolves the current WhatsApp Desktop for Windows build by combining the
 * web JavaScript bundle version with the Microsoft Store package version of
 * the native hybrid shell.
 *
 * <p>The real hybrid shell loads the same JavaScript bundle that
 * {@code web.whatsapp.com} serves, then injects its own store build number
 ...
 * back to a hardcoded plausible build so pairing can still proceed offline
 * or on hosts where outbound access to Microsoft Store is blocked.
 *
 * <p>The {@link ClientAppVersion} returned by {@link #version()} therefore
 * has {@code primary} / {@code secondary} / {@code tertiary} taken from
 * {@link WhatsAppWebClientInfo} (the web bundle the shell hosts) and
 * {@code quaternary} set to the Windows store build integer.
 *
 * @apiNote This class has no direct WA Web counterpart. The real shell
 *          reads its own build number at runtime through native APIs and
 *          passes it through the URL-param contract described above; Cobalt
 *          approximates that path from the outside using the public store
 *          metadata.
...
final class WhatsAppWindowsClientInfo implements WhatsAppClientInfo {
    /**
     * Cached singleton instance of the resolved Windows client info.
     *
    ...
    /**
     * Monitor used to serialise initialisation of {@link #windowsInfo}.
     */
    private static final Object windowsInfoLock = new Object();

    /**
     * Microsoft Store display catalog endpoint for the
     * {@code 9NKSQGP7F2NH} product, which corresponds to the
     * {@code 5319275A.WhatsAppDesktop} UWP package. The response is a JSON
     * document whose {@code Packages[].Version} fields carry the shipping
     * build string.
     */
    ...
    /**
     * JSON marker that precedes the full package name in the display
     * catalog response, for example
     ...
     * ({@code "5319275A.WhatsAppDesktop_cv1g1gvanyjgm"} — family hash, no
     * version) is skipped; the next underscore after the prefix then
     * reliably terminates the dotted-version substring.
     */
    private static final String PACKAGE_MARKER = "\"PackageFullName\":\"5319275A.WhatsAppDesktop_";

    /**
     * Fallback build number used when the Microsoft Store query fails.
     *
     * <p>Chosen to satisfy the {@code WAWebClientPayload.C} length check
     * (six digits so the {@code mcc}/{@code mnc} override fires) and the
     * feature-gate thresholds enforced elsewhere in the Windows hybrid
     * bundle — {@code WAWebHybridUtils} expects {@code >=256700},
     * {@code WAWebWindowsConstants} caps some behaviour at {@code <257700},
     * and {@code WAWebWindowsBuildType} branches on {@code >260000}. The
     * concrete value is captured from a live production handshake so that
     * the offline fallback is indistinguishable from a genuine recent
     * build.
     */
    ...
    /**
     * Returns the cached Windows client info, performing the store query
     * on the first call.
     *
     ...
     * failure is not cached: the next call will retry.
     *
     ...
     * {@link #DEFAULT_WINDOWS_BUILD} fallback is used so that a Cobalt
     * host without outbound access to Microsoft Store can still pair.
     *
     ...
     * {@code 5319275A.WhatsAppDesktop_2.2613.101.0_x64__cv1g1gvanyjgm};
     * the shipping hybrid shell folds this into
     * {@code secondary * 100 + (tertiary % 100)} — the tertiary
     * component encodes a {@code channel}{@code build} pair where the
     * leading digit is the release channel ({@code 1} for
     * {@code RELEASE}) and the trailing two digits are the build
     * counter. For {@code 2.2613.101.0} this yields
     * {@code 2613 * 100 + 1 = 261301}, matching the value observed on
     * the wire from the real {@code WhatsApp.Root.dll} shell.
     *
     * <p>Any failure (network error, missing marker, malformed response)
     * returns {@link #DEFAULT_WINDOWS_BUILD}.
     *
     * @return the resolved six-digit build number, or the fallback
     *         constant when discovery fails
     */
    private static int queryWindowsBuild() {
        try (var httpClient = HttpClient.newBuilder()
            ...
            // Extracts the numeric version segment between the package
            // name prefix and the architecture suffix (e.g. "_x64__...")
            ...
            // Applies the channel-prefix stripping the hybrid shell
            // performs when it builds the windowsBuild URL param: the
            // tertiary's leading digit identifies the release channel
            // and only the low two digits feed the build counter.
    ...
    /**
     * Returns the WhatsApp Desktop for Windows client application
     * version, combining the web JavaScript bundle version with the
     * Microsoft Store package build number in {@code quaternary}.
     *
