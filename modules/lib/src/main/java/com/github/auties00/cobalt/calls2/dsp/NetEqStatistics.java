 ...
 * gross over-buffering; {@link #packetsDiscarded()} counts late or duplicate packets dropped on insert;
 ...
 * @param acceleratedFrames        the count of frames time-compressed by
 *                                 {@link NetEqOperation#ACCELERATE} or
 *                                 {@link NetEqOperation#FAST_ACCELERATE}
 * @param preemptiveExpandedFrames the count of frames time-stretched by
 ...
 * @param bufferFlushes            the count of buffer flushes for over-buffering
 ...
 * @implNote This implementation mirrors the {@code concerto::StatisticsCalculator} counters of the wa-voip
 * WASM module {@code ff-tScznZ8P} ({@code rev-rtc-dsp}: expanded/accelerated/preemptive/discards/
 * buffer_flushes/mean_wait_time/target_delay), the subset the WhatsApp call WAM event needs; the native
 * PeriodicUma loggers that also live on the calculator are telemetry-internal and are not modeled.
 */
