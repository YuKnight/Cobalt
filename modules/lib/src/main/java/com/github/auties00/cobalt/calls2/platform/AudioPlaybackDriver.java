 ...
 *
 * <p>This is the host-side playback-driver contract, the Cobalt replacement for the engine's
 * {@code call_init_playback_driver_js_sync} / {@code call_start_playback_js_sync} /
 * {@code call_stop_playback_js_sync} downcalls. A driver owns one operating-system playback device and
 * runs the same strict {@link AudioDriverState} machine as the capture driver:
 * {@link AudioDriverState#UNINITIALIZED} -> {@link AudioDriverState#INITIALIZED} (after
 * {@link #init(String, int, int, int)}) -> {@link AudioDriverState#ACTIVE} (after {@link #start()}) ->
 ...
 * returned signed 16-bit PCM to the device. This is the Cobalt analogue of the engine's
 * {@code requestAudioData} pull: the host device drives the pace, asking the engine for exactly as many
 * samples as it can play. When the source returns no samples the driver plays silence rather than
 * stalling.
 *
 ...
 * {@link #start()}, {@link #stop()}, {@link #close()}): the owning service drives them in order; the
 * pull from the source runs on the driver's own playback thread.
 *
 * <p>This type is sealed and permits only its production implementation {@link LiveAudioPlaybackDriver},
 * which backs onto a {@code javax.sound.sampled.SourceDataLine}. It supersedes the WhatsApp Web
 * {@code WasmAudioDriver} playback path; the actual writer-thread pump that performs the demand-driven
 * pulls lives in a separate platform service.
 *
 * @apiNote An embedder never implements this interface; the call engine constructs and drives a
 * {@link LiveAudioPlaybackDriver}. The {@link #onRequestAudio(RenderedAudioSource)} source belongs to
 * the engine's decoder, not to application code.
 */
public sealed interface AudioPlaybackDriver permits LiveAudioPlaybackDriver {
     ...
     *
     * <p>This is the engine-to-host handoff for playback samples, the Cobalt analogue of the engine's
     * {@code requestAudioData} pull. The engine installs a source with
     * {@link AudioPlaybackDriver#onRequestAudio(RenderedAudioSource)} and the driver invokes it on its
     * playback thread whenever the device buffer needs filling while
    ...
    interface RenderedAudioSource {
         ...
         * The engine logs {@code Got 0 bytes of audio data!} when a pull yields nothing.
         *
     ...
     * source may be changed at any state.
     *
     ...
     * bind; calling it again in {@link AudioDriverState#INITIALIZED} re-binds with the new parameters.
     ...
     * {@link AudioDriverState#INITIALIZED}; starting from {@link AudioDriverState#UNINITIALIZED} is the
     * recoverable {@code Driver not initialized} fault. Starting an
     * already-{@linkplain AudioDriverState#ACTIVE active} driver is a no-op.
     *
