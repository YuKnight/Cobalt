 ...
 *
 * <p>This is the device-backed {@link AudioOutput} returned by {@link AudioOutput#tone(double)}. Each
 * {@link #take()} renders the next slice of the waveform into a fresh {@link AudioFrame}. Phase is derived
 * from a monotonic frame index rather than reset per frame, so concatenating the emitted frames
 * reproduces one continuous, click-free waveform. A single frequency produces a pure tone; a second
 * non-zero frequency sums two sines, which is how dual-tone signals such as DTMF are built. The stream can
 * also follow an on/off cadence: within each cadence cycle it emits the tone for the configured number of
 * "on" frames and digital silence for the remainder, repeating indefinitely. A stream configured with
 * {@code Long.MAX_VALUE} for both the on and cadence frame counts plays continuously.
 *
 * <p>The stream never ends on its own; like other synthetic sources it produces frames until the call
 * engine shuts it down. Use {@link #sine(int)} for a single test tone, {@link #ringback()} for a Western
 * European ringback while a call is being set up, and {@link #dtmf(char)} to emit a single dial digit.
 *
 * @implNote This implementation is the calls2 port of the legacy tone capture stream; the per-frame
 ...
 */
public final class ToneAudioOutput extends BufferedAudioOutput {
    /**
     * Holds the synthesis sample rate, in Hz, fixed to the call media profile.
     *
    ...
    /**
     * Holds the default duration, in microseconds, of each emitted frame.
     *
     * @implNote This implementation uses 10000, which is 10 ms at {@link #SAMPLE_RATE} for the default
     * {@link #DEFAULT_FRAME_SIZE} frame, in the {@link AudioFrame#ptsMicros()} microsecond clock.
     */
    public static final long DEFAULT_FRAME_DURATION_MICROS = 10_000;
     ...
     * {@code 0x8000}) so that summing two sines for a dual-tone signal cannot exceed full scale and clip.
     */
    ...
    /**
     * Holds the duration of each emitted frame, in microseconds, by which each frame's presentation
     * timestamp advances.
     */
    ...
    /**
     * Constructs a continuous single-tone stream at the given frequency with the default frame geometry.
     *
     * <p>This is the entry point for {@link AudioOutput#tone(double)}; it produces an endless pure sine at
     * {@code frequencyHz} truncated to an integer number of hertz.
     *
     ...
     */
    public ToneAudioOutput(double frequencyHz) {
        this((int) frequencyHz, 0,
                Long.MAX_VALUE, Long.MAX_VALUE,
                DEFAULT_FRAME_SIZE, DEFAULT_FRAME_DURATION_MICROS);
    }
     ...
     * @param frameDurationMicros the duration of each emitted frame in microseconds
     ...
     *                                  {@code 1}, if {@code cadenceFrames} is less than {@code onFrames},
     *                                  or if {@code frameSize} or {@code frameDurationMicros} is less than
     *                                  {@code 1}
     */
    public ToneAudioOutput(double freq1Hz, double freq2Hz, long onFrames, long cadenceFrames,
                           int frameSize, long frameDurationMicros) {
        if (freq1Hz <= 0) {
     ...
     * @throws IllegalArgumentException if {@code frequencyHz} is not greater than {@code 0}
     */
    public static ToneAudioOutput sine(int frequencyHz) {
        return new ToneAudioOutput(frequencyHz, 0,
                Long.MAX_VALUE, Long.MAX_VALUE,
                DEFAULT_FRAME_SIZE, DEFAULT_FRAME_DURATION_MICROS);
    }
     ...
     */
    public static ToneAudioOutput ringback() {
        var onFrames = 1_000_000 / DEFAULT_FRAME_DURATION_MICROS;
        var cadenceFrames = 5_000_000 / DEFAULT_FRAME_DURATION_MICROS;
        return new ToneAudioOutput(425, 0, onFrames, cadenceFrames,
                DEFAULT_FRAME_SIZE, DEFAULT_FRAME_DURATION_MICROS);
    }
     ...
     */
    public static ToneAudioOutput dtmf(char digit) {
        double row, col;
        switch (digit) {
        ...
        return new ToneAudioOutput(row, col,
                Long.MAX_VALUE, Long.MAX_VALUE,
                DEFAULT_FRAME_SIZE, DEFAULT_FRAME_DURATION_MICROS);
    }
