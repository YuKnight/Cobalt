...
/**
 * Decoded table-of-contents (TOC) byte of an MLow packet, the port of {@code smpl_decode_toc} in
 ...
 * sample rate, frame length, rate mode, forward-error-correction (FEC) presence, and channel count. The
 * native decoder unpacks this byte into the {@code smpl_TOC} structure and derives the per-frame loop
 * bounds from it. This record carries the decoded fields plus the derived counts the low-band parameter
 * decoder needs, so a caller never re-derives them from the raw byte.
 *
 ...
 *
 * <p>Two derived fields follow the native conventions in {@code smpl_decode_toc}: {@link #fec()} is set
 * only when {@code VAD} is also set ({@code FEC = VAD && rawBit1}), and {@link #codedAsActiveVoice()} is
 * the disjunction {@code VAD || rawBit1} (so an inactive packet flagged as hangover is still coded as if
 * it may contain voiced energy). The frame-loop counts {@link #numFrames()},
 * {@link #numSubframes()}, and {@link #frameLength16()} mirror the derivations at the top of
 * {@code smpl_core_decode}.
 *
...
public record MlowTocByte(
    ...
    /**
     * Decodes a TOC byte into its fields and derived loop counts, {@code smpl_decode_toc}.
     *
     * <p>Unpacks the eight bit-fields of {@code tocByte} exactly as the native decoder does, including the
     * {@code FEC = VAD && rawBit1} and {@code codedAsActiveVoice = VAD || rawBit1} derivations, and the
     * sample-rate decode {@code (((byte >> 5) & 1) + 1) * 16000}. The low byte is read unsigned, so a
     * caller may pass a sign-extended {@code int} or a raw {@code byte} value.
     *
     ...
     *                                  which the two-bit field cannot produce but is checked to mirror the
     *                                  native assertion
     */
    ...
    /**
     * Returns the low-band frame length in samples at 16 kHz, the native {@code frame_length_16}.
     *
     * <p>Computed as {@code (1 + (packetLenMs > 10)) * (10 * SMPL_CELP_FS_KHZ)}, so a 10 ms packet has a
    ...
    /**
     * Returns the number of internal 20 ms frames in the packet, the native {@code num_frames}.
     *
    ...
    /**
     * Returns the number of subframes per internal frame, the native {@code num_subframes}.
     *
