 ...
 *           equals the group's chaining-salt output. The {@code hbh srtp} label present in the wasm
 *           label table is not derived: on a relayed 1:1 call only the {@code hbh srtcp} masters are
 *           computed, and the media SRTP keys come from the uplink and downlink masters. The two
 *           {@code warp auth} outputs in the wasm label table are likewise not derived here, because
 *           they belong to the relay-authentication schedule rather than to media protection.
 */
