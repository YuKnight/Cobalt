 ...
 * fanned out per device inside the offer's Signal-encrypted {@code <enc>} envelope, then re-exchanged
 ...
 * thread-safe. This class supersedes the legacy {@code call.rtp.srtp.CallKeyDerivation},
 * {@code call.rtp.srtp.HbhKeyDerivation}, and the per-participant base-key portion of
 * {@code call.rtp.sframe.SFrameKeyDerivation}.
 *
 * @implNote This implementation reproduces the per-participant E2E key chain of
 *           {@code call_participant_crypto.cc}: {@code generate_raw_e2e_keys} (fn10890),
 *           {@code derive_sframe_key} (fn10896), {@code generate_sframe_keys_for_participant} (fn10894),
 *           {@code generate_srtp_and_p2p_keys_for_participant} (fn10895), and the keygen-version guard
 *           of {@code call_update_participant_keys} (fn10898, "Unsupported keygen ver"). It also folds
 *           in the SFU hop-by-hop schedule of {@code wa_sfu_kdf} (fn4829). The SFrame base-key schedule
 *           is verified byte-for-byte against live WASM memory paused at {@code derive_sframe_key}
 *           (group call-id {@code 008B72960DAE8E158B0D2201B9A6F98A}): for raw key
 *           {@code 86e0004078464597d59c751fde9a8b61908dcbd04197ffdc7636582be7f439aa} and participant
 *           device JID {@code 83116928594056:2@lid} the base key is
 *           {@code 409102bf2c1a3816c76a6d64819d0c901556e030d5f33da251c13cdfcf0b9353}; the same raw key
 *           with a different device JID yields a different base key. The 32-byte raw-key bound and the
 *           {@value #SUPPORTED_KEYGEN_VER} keygen version are the only "supported" values recovered;
 *           the per-domain ({@code generate_v3_e2e_keys}) variant of the SFrame label is gated on a
 *           group-call-key-exchange flag that was not exercised by any live capture, so this class
 *           leaves the SFrame base key domain-agnostic, matching the live keygen-version-2 path.
 */
public final class CallE2eKeyDerivation {
     ...
     *
     * @implNote This implementation reproduces the {@code 0x20} length written by
     *           {@code generate_raw_e2e_keys} (fn10890) and the upper bound enforced by
     *           {@code call_update_participant_keys} (fn10898).
     */
     ...
     * material; the engine derives keys only for this version and rejects any other.
     *
     * @implNote This implementation reproduces the single "supported" value of
     *           {@code call_update_participant_keys} (fn10898) and
     *           {@code generate_srtp_and_p2p_keys_for_participant} (fn10895), which log "Unsupported
     *           keygen ver %d" for anything else.
     */
     ...
     *
     * @implNote This implementation reproduces the {@code 0x20} output length of {@code derive_sframe_key}
     *           (fn10896).
     */
     ...
     * key slots.
     *
     * @implNote This implementation reproduces the {@code 0x10}-byte per-direction copy of
     *           {@code set_transport_p2p_keys} (fn10892).
     */
     ...
     *
     * @implNote This implementation reproduces the {@code DAT_f0ae4} per-type guard length {@code wa_sfu_kdf}
     *           (fn4829) enforces for the warp-auth key-derivation types (indices {@code 0} and {@code 1}
     *           are both {@code 32}), recovered from the {@code ff-tScznZ8P} data section.
     */
     ...
     *
     * @implNote This implementation reproduces the {@code 16}/{@code 16} split of {@code derive_sframe_key}
     *           (fn10896), confirmed live: salt is the first 16 bytes, input keying material the second.
     */
     ...
     *
     * @implNote This implementation reproduces the 14-byte {@code 'e2e sframe key'} constant of
     *           {@code derive_sframe_key} (fn10896); the live capture confirmed the JID follows it
     *           immediately with no separator and no {@code NUL}.
     */
     ...
     * {@link #deriveWarpAuthKey(byte[])} (the WARP authentication key, 32 bytes) select.
     *
     * @implNote This implementation enumerates the recovered info-label prefixes of {@code wa_sfu_kdf}
     *           (fn4829): {@code 'hbh srtp'}, {@code 'hbh srtcp'}, {@code 'uplink hbh srtcp'},
     *           {@code 'downlink hbh srtcp'}, and {@code 'warp auth'}, each carrying a {@code ' salt'} and a
     *           {@code ' key'} companion in the {@code DAT_f0b0c} label table. The end-to-end
     *           {@code 'e2e sframe key'} and the {@code 'data-channel cert fingerprint hmac'} labels of the
     *           same function are NOT hop-by-hop and are derived elsewhere (the former by
     *           {@link #deriveSframeBaseKey(byte[], CallDeviceJid)}, the latter from the raw key by the
     *           transport layer), so they are deliberately absent here.
     */
    public enum HopByHopGroup {
        /**
         * The non-directional hop-by-hop media SRTP key set, which protects the relayed media RTP on the
        ...
        /**
         * The base non-directional hop-by-hop SRTCP key set, which keys the shared RTCP control traffic.
         */
        SRTCP("hbh srtcp"),
     ...
     * pipeline, and that a group endpoint re-mints on a rekey. It is the input keying material every
     * other method in this chain expands.
     *
     * @return a new {@value #RAW_E2E_KEY_LENGTH}-byte cryptographically-strong raw key
     * @implNote This implementation reproduces {@code generate_raw_e2e_keys} (fn10890), which fills a
     *           {@code 0x20}-byte buffer from the host CSPRNG; here the CSPRNG is
     *           {@link VoipCryptoNative#randomBytes(int)}.
     */
     ...
     * @implNote This implementation reproduces the guard of {@code call_update_participant_keys} (fn10898)
     *           and {@code generate_srtp_and_p2p_keys_for_participant} (fn10895).
     */
     ...
     * @implNote This implementation reproduces {@code derive_sframe_key} (fn10896) and is verified
     *           byte-for-byte against the live WASM memory cited in the class note. The peer SFrame
     *           crypto package owns the per-frame key, salt, nonce expansion and the chain ratchet that
     *           sit on top of this base key; this method derives only the base (chain) key. The legacy
     *           {@code call.rtp.sframe.SFrameKeyDerivation#deriveParticipantBaseKey(byte[], String)}
     *           computes the identical schedule from a raw JID string.
     */
    public static byte[] deriveSframeBaseKey(byte[] rawKey, CallDeviceJid deviceJid) {
        ...
        var jidBytes = deviceJid.jid().toString().getBytes(StandardCharsets.US_ASCII);
     ...
     *                                    bytes long
     * @throws WhatsAppCallException.Srtp if the HKDF computation fails
     * @implNote This implementation reproduces {@code generate_srtp_and_p2p_keys_for_participant}
     *           (fn10895); the legacy {@code call.rtp.srtp.CallKeyDerivation#deriveMaster(byte[], String)}
     *           computes the identical schedule and is verified byte-exact against live cipher
     *           {@code set_key} inputs (for example {@code deriveMaster(callKey, "39110693621863:29@lid")}
     *           against call key {@code bc4e7efa...8ab003d8}).
     */
    public static byte[] deriveSrtpMaster(byte[] rawKey, CallDeviceJid deviceJid) {
        Objects.requireNonNull(deviceJid, "deviceJid cannot be null");
        requireRawKey(rawKey);
        var info = deviceJid.jid().toString().getBytes(StandardCharsets.US_ASCII);
     ...
     * @implNote This implementation reproduces the per-participant derivation order of
     *           {@code call_update_participant_keys} (fn10898): keygen-version guard, then SFrame keys
     *           ({@code generate_sframe_keys_for_participant} fn10894), then SRTP/peer-to-peer keys
     *           ({@code generate_srtp_and_p2p_keys_for_participant} fn10895).
     */
     ...
     * @implNote This implementation reproduces the hop-by-hop branch of {@code wa_sfu_kdf} (fn4829): the
     *           legacy {@code call.rtp.srtp.HbhKeyDerivation} computes the identical chained schedule
     *           and was verified live (each group's key-step salt equals its chaining-salt output).
     */
     ...
     * @throws WhatsAppCallException.Srtp if an HKDF computation fails
     * @implNote This implementation derives the {@code 'warp auth'} group of {@code wa_sfu_kdf} (fn4829)
     *           through the same {@code derive_hbh_srtp_key} (fn4808) two-step chain the SRTP/SRTCP masters
     *           use, the only difference being the {@code DAT_f0ae4} guard length (32 for warp-auth indices
     *           {@code 0}/{@code 1}, versus 30 for the SRTP key-plus-salt groups). The warp-auth derivation
     *           site is reached through an indirect edge not in the static call graph of fn4811 (the only
     *           static fn4808 caller, which derives only the SRTP/SRTCP index pairs), so the {@code <hbh_key>}
     *           input is inferred from the table structure rather than read from a derivation call; it is
     *           near-certain because warp-auth protects the same relay hop the hop-by-hop SRTP does.
     */
    ...
    /**
     * Validates a raw end-to-end key is non-null and exactly {@value #RAW_E2E_KEY_LENGTH} bytes.
     *
    ...
    /**
     * Validates that a byte array is non-null and has an exact length.
     *
     ...
     * fields because the live keygen-version-2 path derives one domain-agnostic SFrame base key and one
     * SRTP master per participant device; the per-domain split, when a future keygen version introduces
     * it, is computed locally and never transmitted.
     *
     ...
     *                      salt)
     * @implNote This implementation models the per-participant outputs of
     *           {@code call_update_participant_keys} (fn10898) as an immutable record rather than the
     *           native crypto block embedded at {@code participant+0x61c78}.
     */
