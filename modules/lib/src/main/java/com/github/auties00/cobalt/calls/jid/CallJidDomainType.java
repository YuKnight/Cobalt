...
/**
 * Enumerates the voip JID domain type codes the call engine assigns to a callable JID.
 *
 * <p>The engine classifies every JID it handles into a small integer domain type code
 * stored alongside the JID; that code, not the textual server string, is what the call
 * layer branches on when deciding whether a JID is a phone number user, a group, a LID, a
 * bot, a hosted device, and so on. Each constant binds a code to its
 * {@linkplain #domainString() canonical server string} and to the equivalent Cobalt
 * {@link JidServer.Type}.
 *
 ...
 * variant slots that also resolve to {@code s.whatsapp.net}. The remaining codes are
 * distinct domains: {@code 1} group ({@code g.us}), {@code 3} call ({@code call},
 * extension and link), {@code 5} LID user or device ({@code lid}), {@code 8} newsletter
 * ({@code newsletter}), {@code 9} bot ({@code bot}), and {@code 11} hosted LID device
 * ({@code hosted.lid}). The domain predicates read directly off these codes: a JID is a
 * LID when its code is {@code 5}, a bot when its code is {@code 9}, and a hosted device
 * when its code is {@code 11}; a device JID counts as LID addressed when its code is
 * {@code 5}, {@code 9}, or {@code 11}, and resolving a hosted LID device to its owning
 * user maps code {@code 11} back to code {@code 5}.
 */
public enum CallJidDomainType {
    ...
    /**
     * The integer domain type code the call engine stores for this domain.
     */
    ...
    /**
     * Returns the integer domain type code the call engine stores for this domain.
     *
