...
/**
 * Two-stage line-spectral-frequency (LSF) inverse vector quantizer for the MLow speech codec, the decode
 * subset of {@code smpl_lsf_quant.c}.
 *
 ...
 * decode-side inverse of that quantizer. It does two things, exposed as the two halves of one
 ...
 * from the range coder using the LSF cumulative mass functions, the port of the LSF portion of
 * {@code smpl_decode_lb_params}.</li>
 * <li>{@link #dequantize(int[], int, int, float[])} reconstructs the per-frame LSF vector from those
 * indices, the port of {@code smpl_lsf_dequant}.</li>
 * </ul>
 *
 ...
 * non-conditional path doubles the chosen half-centroid and applies that centroid's forward weighting
 ...
 *
 * <p>Scope is the 16 kHz / 60 ms / mono SMPL low-band decode path. The encoder-only quantization and
 * survivor search ({@code smpl_lsf_quant} / {@code smpl_lsf_quant_cond} / {@code VQ_temp}) are not ported.
 * This type is stateless and thread-safe; the caller threads the previous-frame LSF vector explicitly.
 *
 * @implNote This implementation ports {@code smpl_lsf_dequant}, {@code SMPL_lsf_min_dist},
 * {@code smpl_lsf_weights_laroia}, and the matrix-transpose multiply {@code smpl_matrix_mult_transp_16}
 * from {@code smpl_lsf_quant.c} and {@code smpl_codec_util.c}, plus the LSF index-decode steps of
 * {@code smpl_decode_lb_params} from {@code smpl_param_coding.c}. The matrix multiply accumulates the
 * {@code j == 0} term first and then ascending {@code j}, matching the C order so the non-associative
 * {@code float} sums round identically. The minimum-distance relaxation replays the {@code 1000}-iteration
 * fixed-point loop verbatim, including the {@code k * 1.0e-6f - dm} step schedule.
 */
public final class LsfDequantizer {
     ...
     *
     * <p>Each inverse spacing is computed over a gap clamped up to this value so a near-coincident pair of
     * line spectral frequencies cannot produce an unbounded weight.
     */
    private static final float LAROIA_MIN_DIST = 1e-3f;

    /**
     * Upper band edge for the LSF range, the {@code SMPL_PI} constant.
     *
     * <p>This is the {@code float}-rounded value of pi the reference uses; both the Laroia weighting and
     * the minimum-distance relaxation measure the top spacing against this edge, so the exact constant is
     * load-bearing for bit-near reconstruction.
     */
     ...
     *
     * <p>The indices are the bit-exact gate against the native decoder; {@code lsf} is the
     * single-precision reconstruction the linear-prediction interpolation consumes and is also threaded
     * back as the previous-frame vector for the next conditionally coded frame.
     *
     * @param indices the {@code SMPL_LPC_ORDER + 1} decoded indices: {@code indices[0]} is the stage-1
     *                centroid in {@code [0, LSF_CB_CENTROIDS]}, and {@code indices[i + 1]} is the stage-2
     *                level index for coefficient {@code i}
     * @param lsf     the {@code SMPL_LPC_ORDER} reconstructed line spectral frequencies, strictly
     *                increasing within {@code (0, SMPL_PI)}
     */
     ...
     * {@link #dequantize(int[], int, int, float[])}. The two halves are exposed separately for callers that
     * only need one; this method is the convenience that performs both.
     *
     * @param decoder     the range decoder positioned at the frame's stage-1 LSF symbol; advanced past all
     *                    {@code SMPL_LPC_ORDER + 1} LSF symbols on return
     ...
     *                    centroid is selected; {@code SMPL_LPC_ORDER} entries
    ...
    public DecodedLsf decode(MlowRangeDecoder decoder, int voiced, int lowRate, boolean conditional,
                             float[] previousLsf) {
        int[] indices = decodeIndices(decoder, voiced, lowRate, conditional);
        float[] lsf = dequantize(indices, voiced, lowRate, previousLsf);
        return new DecodedLsf(indices, lsf);
    }

    /**
     * Reads the stage-1 and stage-2 LSF indices from the range coder, the LSF portion of
     * {@code smpl_decode_lb_params}.
     *
     * <p>The stage-1 index is decoded against the centroid cumulative mass function for the class, choosing
     * the conditional-coding table when {@code conditional} is set. Each of the {@value #LPC_ORDER} stage-2
     * indices is then decoded against the per-coefficient cumulative mass function of the selected stage-1
     * centroid, drawn from the {@code (voiced, lowRate, centroid)} stage-2 sub-codebook. The decoder is
     * advanced past every symbol.
     *
     ...
     * @return a freshly allocated array of {@code SMPL_LPC_ORDER + 1} indices: stage-1 at {@code [0]},
     *         stage-2 levels at {@code [1, SMPL_LPC_ORDER]}
     */
    public int[] decodeIndices(MlowRangeDecoder decoder, int voiced, int lowRate, boolean conditional) {
        Stage1 st1 = codebook.stage1(voiced);
        int[] stage1Cmf = conditional ? st1.cmfCond() : st1.cmf();
        int[] indices = new int[LPC_ORDER + 1];
        indices[0] = MlowEntropyWrapper.decodeUpdate(decoder, stage1Cmf);
        Stage2 st2 = codebook.stage2(voiced, lowRate, indices[0]);
    ...
    /**
     * Reconstructs the per-frame LSF vector from the decoded indices, the port of
     * {@code smpl_lsf_dequant}.
     *
     ...
     *
     * @param indices     the {@code SMPL_LPC_ORDER + 1} indices from
    ...
    public float[] dequantize(int[] indices, int voiced, int lowRate, float[] previousLsf) {
        Stage1 st1 = codebook.stage1(voiced);
        int centroid = indices[0];
        Stage2 st2 = codebook.stage2(voiced, lowRate, centroid);
        ...
        if (centroid == CB_CENTROIDS) {
            float[] mean = st1.mean();
            float regCond = st1.regCond();
            for (int i = 0; i < LPC_ORDER; i++) {
                lsfq1[i] = previousLsf[i] + regCond * (mean[i] - previousLsf[i]);
            }
            float[] lsfw = laroiaWeights(lsfq1);
            for (int i = 0; i < LPC_ORDER; i++) {
        ...
        float[] lsfq = new float[LPC_ORDER];
        for (int i = 0; i < LPC_ORDER; i++) {
     ...
     *
     * <p>The reference flattens the matrix row-major and indexes element {@code (j, i)} as
     * {@code C[j * 16 + i]}, so the multiply transposes the supplied matrix. The accumulation initializes
     * each output with the {@code j == 0} term and then adds ascending {@code j} terms in place; this order
     * is preserved because {@code float} addition is not associative and the reference rounds in exactly
     * this sequence.
     *
    ...
    /**
     * Enforces the minimum-distance ordering on a reconstructed LSF vector in place, {@code SMPL_lsf_min_dist}.
     *
