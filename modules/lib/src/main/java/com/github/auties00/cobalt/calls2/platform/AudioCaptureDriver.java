 ...
 * local audio from.
 *
 * <p>This is the host-side capture-driver contract, the Cobalt replacement for the engine's
 * {@code call_init_capture_driver_js_sync} / {@code call_start_capture_js_sync} /
 * {@code call_stop_capture_js_sync} downcalls. A driver owns one operating-system capture device and
 * runs the strict {@link AudioDriverState} machine over it: {@link AudioDriverState#UNINITIALIZED} ->
 * {@link AudioDriverState#INITIALIZED} (after {@link #init(String, int, int, int, AudioDeviceType)})
 * -> {@link AudioDriverState#ACTIVE} (after {@link #start()}) -> {@link AudioDriverState#INITIALIZED}
 * (after {@link #stop()}). While {@linkplain AudioDriverState#ACTIVE active} the driver reads signed
 * 16-bit PCM from its device and forwards each captured block to the {@link CapturedAudioSink}
 * installed with {@link #onCapturedAudio(CapturedAudioSink)}, which the engine wires to its encoder.
 *
 ...
 * the sample delivery to the sink runs on the driver's own capture thread.
 *
 * <p>This type is sealed and permits only its production implementation {@link LiveAudioCaptureDriver},
 * which backs onto a {@code javax.sound.sampled.TargetDataLine}. It supersedes the WhatsApp Web
 * {@code WasmAudioDriver} capture path; the actual reader-thread pump that pulls fixed-size chunks lives
 * in a separate platform service.
 *
...
public sealed interface AudioCaptureDriver permits LiveAudioCaptureDriver {
     ...
     *
     * <p>This is the host-to-engine handoff for captured samples, the Cobalt analogue of the engine's
     * {@code send_to_audio_sink} call inside {@code WasmVoipAVDriverManager::processAudioCaptureData}.
     * The engine installs a sink with {@link AudioCaptureDriver#onCapturedAudio(CapturedAudioSink)} and
     * the driver invokes it for every block it captures while
     * {@linkplain AudioDriverState#ACTIVE active}, on the driver's capture thread.
     */
    @FunctionalInterface
     ...
     * {@link AudioDriverState#INITIALIZED} re-binds with the new parameters. The driver must not be
     ...
     * initialized} fault. Starting an already-{@linkplain AudioDriverState#ACTIVE active} driver is a
     * no-op.
     *
     ...
     * {@link #start() started} again without re-initializing. Stopping a driver that is not
     ...
     *
     * <p>Mirrors the engine's {@code set_device_type}: it updates the {@link #deviceType()} used to tag
     * captured blocks and to select the capture path on the next {@link #start()}. The driver must not be
     * {@linkplain AudioDriverState#ACTIVE active}, since changing the source kind of a running capture is
     * undefined; the engine logs an override warning in that case.
     *
     ...
     * {@link AudioDriverState#UNINITIALIZED}. The driver may be re-bound with
