 ...
 * capability bitset.
 *
 * <p>Each constant names one bit position inside the per-version capability mask that the
 * wa-voip engine packs and serializes into call signaling (the {@code voip_capability}
 * child of {@code offer}, {@code preaccept}, {@code accept}, and {@code link_join}). A
 * capability is identified by a packed value combining a bit {@link #index() index} within
 * the version mask and the {@link #sinceVersion() version} the bit was introduced in; the
 * engine packs that pair as {@code index | (version << 16)} when adding a self-capability
 * and tests it as {@code (mask[index >> 3] >> (index & 7)) & 1} against a peer's advertised
 * mask. The index range is {@code 0} through {@link #MAX_INDEX} inclusive for every version.
 *
 * <p>The constants here are the bits that make up the {@linkplain VoipCapabilities#standard()
 * standard self-advertisement} the reference client emits: the twenty-two version {@code 1}
 * bits at indices {@code 0, 1, 2, 4, 5, 6, 7, 8, 11, 18, 21, 22, 23, 24, 25, 27, 28, 29, 31,
 * 32, 33, 36}. The semantic meaning of each individual bit (which feature it gates) is
 * assigned by the signaling layer that calls the engine's capability-add entry point and is
 * not encoded in the capability module itself, so each constant is named positionally
 * ({@code BIT_<index>}) until the feature-to-index mapping is recovered.
 *
 * @implNote This implementation enumerates the bits recovered by decoding the captured
 * standard advertisement {@code 01 05 F7 09 E4 BB 13} emitted by the wa-voip WASM module
 * {@code ff-tScznZ8P}. That byte stream is the output of {@code wa_serialize_voip_capabilities}
 * (fn10573, {@code voip_capabilities.cc}): a leading top-version byte {@code 0x01}, then for
 * version {@code 1} a mask-length byte {@code 0x05} and the five least-significant-bit-first
 * mask bytes {@code F7 09 E4 BB 13}, which expand to the twenty-two set bit indices listed
 * above. The {@link #MAX_INDEX} bound {@code 0x1ff} and the per-version mask width are taken
 * from {@code self_capabilities_add} (fn10577) and {@code pjmedia_voip_capabilities_create}
 * (fn10567), which allocate a {@code 0x40}-byte mask and reject any index outside
 * {@code 0..0x1ff} (the runtime guard "Capability index %d out of range, range allowed %d -
 * %d"). The signaling sender {@code fill_voip_capability_bitfield} (fn11452) writes the
 * advertisement header {@code 0x3f00000001} (version {@code 1}, {@code 0x3f} entries),
 * confirming the version this set belongs to.
 */
