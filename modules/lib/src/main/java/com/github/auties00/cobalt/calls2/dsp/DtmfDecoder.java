 ...
 * concealment of a lost telephone-event packet mid-tone. An event code outside the synthesizable range, or
 ...
 *
 * @implNote This implementation ports the RFC 4733 parsing {@code webrtc::DtmfBuffer::ParseEvent} (fn9930)
 * of the wa-voip WASM module {@code ff-tScznZ8P} ("ParseEvent payload too short") and the {@code kDtmf}
 * tone synthesis {@code GetAudioInternal} (fn7521) dispatches to {@code webrtc::DtmfToneGenerator}: the
 * four-byte event header (event, E/R bits and volume, 16-bit duration) selects the low/high tone pair. The
 * tone is the WebRTC second-order sinusoid recursion {@code y[n] = (a*y[n-1] >> 14) - y[n-2]} with
 * {@code a = 2*cos(2*pi*f/fs)} in Q14, seeded with {@code x[-1] = 0}, {@code x[-2] = sin(2*pi*f/fs)}; the
 * {@link #COEFF_LOW}, {@link #COEFF_HIGH}, {@link #INIT_LOW}, {@link #INIT_HIGH}, and {@link #AMPLITUDE}
 * tables and the {@link #AMP_MULTIPLIER} 3 dB low-tone attenuation are the verbatim WebRTC values, present
 * as int32 in the WASM data segment {@code data-segment-0088.bin}. The native generator has no onset or
 * offset amplitude ramp; the resonator initialization gives a clean tone start and the recursion memory
 * carries across frames so a sustained tone is continuous.
 */
public final class DtmfDecoder implements AudioDecoder {
     ...
     *
     * <p>A shorter payload is rejected as malformed, matching the native "ParseEvent payload too short"
     * guard.
     */
     ...
     *
     * <p>Tracked so the oscillator is re-seeded only when the event or volume changes, keeping the
     ...
     * rate falls back to the 16 kHz row, matching the native generator default.
     *
     ...
     * synthesizes one frame. The end flag or an out-of-range event code stops the tone and yields silence.
     * The {@code fec} flag is ignored.
     */
    @Override
