...
/**
 * Drives the in-call screen-share control: starting, stopping, and tracking a screen-share stream.
 *
 ...
 * behind a lock and is bound to one call's identity and its signaling and event seams at construction; it
 * owns no timers, so it needs no explicit shutdown.
 *
 * @implNote This implementation reproduces the screen-share control of the wa-voip WASM module
 * {@code ff-tScznZ8P} ({@code media/screen_share.cc}): {@code start}/{@code stop}/{@code fail} send the
 * {@code <screen_share>} element (type {@code 38}) with {@code screenshare_state} {@code 1}/{@code 2}/{@code 3}
 * and the negotiated {@code version}, and emit event {@code 0x74} ({@code ScreenShare}) through
 * {@code send_screen_share_event} (state, sharer_jid, version, reason). The V2-versus-V3 split is the
 * single-stream port swap versus the dual-stream auxiliary media stream. The info-mutex is replaced by a
 * {@link ReentrantLock} per the threading design.
 */
