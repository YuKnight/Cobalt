 ...
 *
 * <p>DCEP is the in-band handshake that opens a WebRTC DataChannel over an already-established SCTP
 * association: one side sends a {@link Open} ({@code DATA_CHANNEL_OPEN}) on the channel's stream carrying
 * the channel label, subprotocol, priority, and reliability parameters, and the peer answers with a
 * single-byte {@link Ack} ({@code DATA_CHANNEL_ACK}). Both ride the SCTP association on the DCEP Payload
 * Protocol Identifier {@link #PPID_DCEP}, distinct from the application-data PPIDs the channel uses once
 * open. This sealed interface has exactly those two variants; {@link #encode()} produces the bytes to
 * send on {@link #PPID_DCEP} and {@link #decode(byte[])} parses the payload of an inbound DCEP chunk back
 * into the matching typed variant.
 *
 ...
 *
 * @implNote This implementation reimplements the DCEP framing the WhatsApp Web build offloads to its
 * host data-channel controller ({@code transport/data_channel/wa_data_channel_controller_wrapper.cc} in
 * the wa-voip WASM module {@code ff-tScznZ8P}, whose ICE/DTLS/DCEP machinery is host-provided). Cobalt
 * binds usrsctp directly and frames DCEP in Java reusing {@code model.call.datachannel}; the wire
 * constants are the IANA-registered RFC 8831 PPID and the RFC 8832 message- and channel-type bytes.
 */
public sealed interface DcepMessage {
     ...
     * type, and the UTF-8 label and subprotocol strings. The compact constructor rejects an out-of-range
     * priority or reliability parameter and a label or protocol longer than the 16-bit length field can
     * express.
     *
     ...
     *                             retransmits for a retransmit-limited channel, or the maximum lifetime in
     *                             milliseconds for a timed channel
     * @param label                the channel label, UTF-8 encoded
     * @param protocol             the application-level subprotocol identifier, UTF-8 encoded and possibly
     *                             empty
     */
    record Open(byte channelType,
         ...
         * parameter, and a label or protocol whose UTF-8 form exceeds the 16-bit length field.
         *
         ...
         *                                  the label or protocol exceeds 65535 UTF-8 bytes
         */
        public Open {
         ...
         * channel type is not lifetime-limited.
         *
         ...
         * {@inheritDoc}
         *
         * <p>Serialises the {@code DATA_CHANNEL_OPEN} header followed by the UTF-8 label and protocol
         * bytes, all big-endian, into a freshly allocated array.
         *
         ...
         * already verified.
         *
         * <p>Reads the fixed {@link #OPEN_HEADER_SIZE}-byte header, then the label and protocol strings
     ...
     * message-type byte. The type has no per-instance state; callers reuse the cached {@link #INSTANCE}
     * rather than allocating fresh values.
     */
