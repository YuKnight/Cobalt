 ...
 * payloads are SFrame-encrypted by the sender, the relay or SFU forwards the ciphertext without
 ...
 *
 * <p>The base key is a single HKDF-SHA256 output. The 32-byte call key is split into two 16-byte
 * halves: the first half is the HKDF salt and the second half is the HKDF input keying material. The
 * info parameter is the fixed ASCII label {@value #INFO_LABEL_STRING} immediately followed, with no
 * separator and no trailing {@code NUL}, by the participant's device JID in raw ASCII:
 *
 * {@snippet :
 ...
 *
 * @implNote This implementation reproduces {@code derive_sframe_key} (fn10896 in the reversed
 * snapshot {@code ff-tScznZ8P}, relocated to live fn11063 in {@code RngUzmbtgHS}) from
 * {@code core/call_participant_crypto.cc}. The native routine builds a {@code 0x91}-byte info buffer
 * (label {@value #INFO_LABEL_LENGTH} bytes plus the participant context, {@code strnlen}-bounded to
 * {@code 0x50}=80) and calls the host HKDF callback with {@code salt = secret[0..16]},
 * {@code IKM = secret[16..32]}, {@code outLen = 0x20}. The schedule is verified BYTE-FOR-BYTE against
 * live WASM memory captured while paused at {@code derive_sframe_key} during a group rekey
 * (re/calls2-spec/captures/group-sframe-frame.json): for
 * {@code secret = 86e0004078464597d59c751fde9a8b61908dcbd04197ffdc7636582be7f439aa} and
 * participant {@code "83116928594056:2@lid"} the derived base key is
 * {@code 409102bf2c1a3816c76a6d64819d0c901556e030d5f33da251c13cdfcf0b9353}; the swapped-salt/IKM and
 * the {@code +NUL}-separator variants both fail to match, fixing salt as the first half and the info
 * as label-immediately-followed-by-JID. The HKDF itself is delegated to
 * {@link VoipCryptoNative#hkdfSha256(byte[], byte[], byte[], int)} (the JCA
 * {@code KDF("HKDF-SHA256")}), the same primitive the native engine reaches through statically-linked
 * BoringSSL.
 */
public final class SFrameKeyDerivation {
     ...
     *
     * <p>The native rekey driver rejects a raw end-to-end key longer than {@code 0x20}
     * ({@code call_update_participant_keys}, fn10898); this derivation requires exactly that length.
     */
     ...
     *
     * <p>The native routine appends the participant JID through a {@code strnlen} bounded to
     * {@code 0x50} (80) bytes; a longer JID is truncated to this length before it enters the info.
     */
    public static final int MAX_CONTEXT_LENGTH = 0x50;

    /**
     * Holds the fixed ASCII info label as a string, for documentation cross-reference.
     */
