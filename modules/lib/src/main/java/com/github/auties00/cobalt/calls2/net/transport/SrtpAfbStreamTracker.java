 ...
 *
 * @implNote This implementation reproduces {@code hbh_srtp_create_srtp_afb_info} (fn4821) in
 *           {@code transport/wa_hbh_srtp_relay.cc}, which builds an {@code SrtpAfbStreamInfo}
 *           {@code {ssrc, rtp_index u64, rtcp_index u32, is_valid}} per tracked stream. Cobalt holds
 *           the live per-stream indices in a map keyed by SSRC and materialises the model
 *           {@link SrtpAfbStreams} on demand; the indices monotonically advance, mirroring the native
 *           ROC-extended {@code rtp_index} and SRTCP {@code rtcp_index} watermarks.
 */
