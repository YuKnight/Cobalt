...
/**
 * Factory for creating {@link SSLEngine} instances configured with a
 * TLS fingerprint that matches a real browser.
 */
public final class WhatsAppSslEngineFactory {
    /**
     ...
     */
    static final String[] CHROME_CIPHER_SUITES = {
    ...
    private WhatsAppSslEngineFactory() {
        throw new UnsupportedOperationException("WhatsAppSslEngineFactory is a utility class and cannot be instantiated");
    }

    /**
     * Creates a Chrome-like {@link SSLEngine} for the given peer address.
     *
     * <p>The engine is configured with ALPN ({@code http/1.1}), HTTPS
     * hostname verification, and Chrome's cipher suite ordering.
     *
     * @return a lambda that configures {@link SSLEngine} in client mode
     */
    public static Function<InetSocketAddress, SSLEngine> newWebSSLEngineFactory() {
        return address -> {
            try {
