 ...
 * transport thread.
 *
 * @implNote This implementation ports the machine-learning-congestion-driven controller from the
 * wa-voip engine ({@code bwe/wa_ml_cong_pid_controller.cc}), which wraps the generic
 * {@code wa_pid_controller.cc} (fn4540) to map the congestion model output into a rate adjustment. The
 * gains and the minimum factor are read from the voip parameters; the captured {@code voip_settings}
 * blob does not carry those literals, so they are supplied by the caller
 * (re/calls2-spec/SPEC.md sec 15.5).
 */
public final class MlCongestionPidController {
     ...
     *
     * <p>Bounds how far one adjustment can back the rate off, clamping the factor's lower end.
     */
    private final double minFactor;

    /**
     * Most recently computed rate-scaling factor.
     */
