 ...
 *
 * <p>The integers are LEB128: little-endian base-128 with a high-bit continuation flag, at most ten
 * bytes for a 64-bit value. The trailer length therefore ranges from {@value #MIN_TRAILER_LENGTH}
 * (a one-byte key id, a one-byte counter, and the length byte) up to {@value #MAX_TRAILER_LENGTH}.
 *
 * @implNote This implementation reproduces {@code sframe_write_varint} (fn6941) and
 * {@code sframe_read_varint} (fn6942) and the trailer assembly in {@code wa_sframe_encrypt} (fn6898)
 * and {@code wa_sframe_decrypt} (fn6900) from {@code wa_sframe_secureframe_c_api.cc} of the wa-voip
 * WASM module {@code ff-tScznZ8P}. The CRITICAL field order is key-id-before-counter, pinned at the
 * instruction level: in {@code wa_sframe_encrypt} (reversibility/instructions.jsonl fn6898) the first
 * value byte-written by {@code sframe_write_varint} is the same value passed to {@code fn6948}
 * ({@code get_or_derive_cipher}, which keys the cipher cache by key id), i.e. the key id, and the
 * second written value is the counter; the closing byte is {@code len1 + len2 + 1} (instructions
 * {@code at} 2841938-2841977). Symmetrically {@code wa_sframe_decrypt} (fn6900) reads the first varint
 * and passes it to {@code fn6948} (the key id) and reads the counter second (decompile lines 695-725).
 * This corrects the reversed {@code [counter][keyId]} order of the superseded
 * {@code call/rtp/sframe/SFrameSecureFrame}; it is the single most important SFrame trailer
 * correction. The order is fully determined by the static code, which is byte-stable across the live
 * revision drift (the encrypt function body is 806 bytes, prologue-identical to the snapshot with only
 * relocated call indices differing). The order is still STATIC-ONLY: it could not be confirmed against
 * a live frame because {@code wa_sframe_encrypt} is never invoked on any observed WA Web media path. A
 * live capture on 2026-06-15 (re/calls2-spec/captures/sframe-frame-live.json) connected a 3-party SFU
 * group VIDEO call with video provably encoding at 14 fps and proved raw-CDP worker-breakpoint reach
 * (it paused {@code derive_sframe_key}), yet neither {@code wa_sframe_encrypt} nor its video caller
 * {@code wa_video_sframe_encode_cb} ever fired: SFU group-call media is relay hop-by-hop SRTP and the
 * SFrame per-frame transform is not engaged, while a 1:1 call carries no SFrame layer at all.
 */
public final class SFrameHeaderCodec {
    ...
    /**
     * Holds the payload mask of a LEB128 byte: the low seven value-carrying bits.
     */
     ...
     * continuation bit clear, capping its scan at {@value #MAX_VARINT_LENGTH} bytes. A truncated or
     * over-long varint (no terminating byte within {@code limit} or within
     * {@value #MAX_VARINT_LENGTH} bytes) is reported as a zero consumed length, which the caller
     * treats as a malformed trailer.
     *
     ...
     * @return the number of bytes consumed, or {@code 0} if the varint is truncated or over-long
