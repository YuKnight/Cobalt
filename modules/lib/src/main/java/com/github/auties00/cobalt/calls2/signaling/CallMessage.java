 ...
 * knows both how to render itself back to a {@link Node} ({@link #toNode()}) and which taxonomy entry it
 ...
 * tag alone. Inbound dispatch in {@link Calls2CallStanza#parse(Node)} therefore keys on the child
 * element tag through {@link Node#description()}, not on {@link #type()}, and {@link #type()} is a
 ...
 * decoded form as a record per action rather than a single flat C struct, and reuses {@code node/binary}
 * verbatim for the wire bytes, so this interface carries only the two operations every action needs:
 * {@link #toNode()} (the {@code serialize_*} side, over the common header stamped by
...
public sealed interface CallMessage permits
     ...
     * on the wire child tag through {@link Calls2CallStanza#parse(Node)} rather than on this projection,
    ...
    /**
     * Renders this message to its {@code <call>} child element {@link Node}.
     *
     * <p>The returned node is the action element only: it carries the action's wire tag, its universal
     ...
     * responsibility, so the same action node can be addressed to different recipients without rebuilding
     * it.
     *
     * @return the action element node; never {@code null}
     */
    Node toNode();
}
