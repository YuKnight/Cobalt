...
/**
 * The minimal per-call identity an in-call control operation needs to address its signaling.
 *
 ...
 * context itself.
 *
 * @implNote This implementation carries the fields the wa-voip in-call action serializers read from the
 * call context when stamping the common header in {@code populate_common_call_attr} (fn11591) of module
 * {@code ff-tScznZ8P}: the {@code call-id} (data offset {@code 0x888f9}) and the {@code call-creator}
 * device JID (data offset {@code 0x45ea5}), plus the local device JID and the group flag the controllers
 * branch on. It is a deliberate Cobalt seam: the native engine reaches these through the {@code call_context}
 * blob a controller is invoked against, whereas Cobalt passes the projected identity so the control
 * package does not depend on the lifecycle package's mutable context type.
 ...
 *                    actions and selects fan-out addressing
 */
public record CallControlContext(String callId, Jid callCreator, Jid selfJid, boolean group) {
    ...
    /**
     * Returns a one-to-one (non-group) control context for the given call identity.
     *
     * <p>This is the convenience used for a one-to-one call, where the group-only actions do not apply;
     * it sets {@link #group()} to {@code false}.
     *
