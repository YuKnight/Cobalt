 ...
 * the end of the stream. The face the application drives is {@link #read()}, by which the embedder
 * pulls received frames to render them; a file backed sink instead writes each frame to disk inside
 * {@link #offer(VideoFrame)} and is not read from.
 *
 ...
 * {@link #toY4m(Path)} to record it to a raw Y4M file. The {@link #offer(VideoFrame)} and
 * {@link #shutdown()} methods belong to the engine; application code drives a custom sink through
 * {@link #read()} and never calls the pair the engine drives directly.
 */
public interface VideoInput {
    /**
     * Returns a sink that discards the received video.
     *
     * <p>Every {@link #offer(VideoFrame)} is dropped and {@link #read()} yields nothing, so this is the
     * sink to install when a call needs no inbound video, and is the default video sink. {@link #read()}
     ...
     *
     * <p>Each {@link #offer(VideoFrame)} appends the frame uncompressed, so no encoder is involved and a
     ...
     * {@link #read()} or hands it to another thread copies the pixels out first (or copies the whole frame),
