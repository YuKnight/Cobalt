...
/**
 * Encoder per-frame voiced/unvoiced signal-mode classifier of the MLow speech codec, the port of
 * {@code smpl_get_signal_mode.c} ({@code smpl_get_signal_mode} and its static {@code smpl_inv_sigmoid}).
 *
 ...
 * returned strength, gated by the packet's {@code coded_as_active_voice} flag, is the native
 * {@code enc_state->voiced_buf[numframe]} decision that selects the encoder coding path: a positive strength on
 * an active-voice packet marks the frame voiced, so the parameter encoder codes the adaptive-codebook pitch lag
 * and runs the voiced analysis-by-synthesis search; a non-positive strength marks the frame unvoiced, so the
 * pitch lags are zeroed and the unvoiced fixed-codebook plus residual-energy path is taken instead. The
 * magnitude of the strength is also retained by the core encoder as {@code voicing_strength_buf} and feeds the
 * bit-rate controller.
 *
 ...
 *   <li><b>Voice-activity strength.</b> A monotone map of the SILK speech-activity probability into roughly
 ...
 *       {@code [-1, 1]}; a low-frequency-dominated spectrum pulls towards voiced.</li>
 ...
 *
 * <p>This classifier is stateful across the frames of a stream through the native {@code VUV_Mode} record it
 * carries internally: the hysteresis copy of the previous strength, the previous frame's last-subframe lag, and
 * the smoothed low-band and high-band background energy floors. All four start at zero, the native
 * {@code memset(enc_state, 0, ...)} initial state, and {@link #reset()} restores exactly that. Construct one
 * classifier per logical stream, feed it every frame in order, and reset it between independent streams. This
 ...
 * spectrum.
 *
 * @implNote This implementation reproduces the exact arithmetic of the {@code -Ofast -mavx} build of
 * {@code smpl_get_signal_mode.c}, which is a fast-math (associative, flush-to-nothing) translation unit. The
 * two spectral-tilt energy sums are auto-vectorized by GCC into four-lane packed-single accumulators with the
 * integer index weights computed and converted in-lane, so {@link #spectralTiltEnergyLow} and
 * {@link #spectralTiltEnergyHigh} accumulate as the native object does: lane {@code k} holds the running sum of
 * {@code ((knee +/- i) * (i + 3)) * F2[i]} and the lanes reduce as {@code (lane0 + lane2) + (lane1 + lane3)},
 * with the low-band sum finishing on a three-element scalar tail. The fast-math reassociations the build
 * performs elsewhere are mirrored verbatim: the tilt denominator associates as {@code (low + eps) + high}, the
 * cubed-tilt numerator term as {@code (tilt * weight) * (tilt * tilt)}, the inverse-sigmoid negation and the
 * lag sigmoid are folded into the final weighted sum as subtractions, and the bias is added as the negative
 * literal. A plain left-to-right rewrite differs in the last float bit and can flip the voiced decision, which
 * changes the entire coded frame, so the order here is load-bearing.
 */
final class SignalModeClassifier {
    /**
     * Linear-prediction power-spectrum bin count, the native {@code SMPL_F_LEN}; the length of the {@code F2}
     * magnitude-squared spectrum the LPC analysis produces and this classifier reads.
     */
    private static final int F2_LEN = 257;

    /**
     * Number of lag subframes per 20 ms frame in the locked high-rate scope, the native
     * {@code framelen / SMPL_LAG_SUBFRLEN} for a 320-sample frame over 40-sample lag subframes.
     *
    ...
    /**
     * Index splitting the low-band and high-band spectral-tilt sums, the native
     * {@code TRANSITION_IX = SMPL_F_LEN / 3}.
     *
     * <p>The low-band sum runs over bins {@code [2, TRANSITION_IX)} and the high-band sum over
     * {@code [TRANSITION_IX, SMPL_F_LEN)}; the integer truncation of {@code 257 / 3} fixes this at {@code 85}.
     */
    private static final int TRANSITION_IX = F2_LEN / 3;

    /**
     * Voicing decision bias, the native {@code SMPL_VUV_BIAS}.
     *
    ...
    /**
     * Hysteresis weight on the previous frame's strength, the native {@code SMPL_VUV_HYST}.
     *
     * <p>Scales the lag-jump-decayed copy of the previous frame's voicing strength before it is added to the
     * current frame's strength, smoothing the voiced/unvoiced decision across frames.
     */
    private static final float VUV_HYST = 0.05f;

    /**
     * Feature weights on pitch correlation, voice activity, spectral tilt, harmonicity, and short lags, the
     * native {@code smpl_vuv_weights} (first five entries).
     *
     * <p>The weighted feature sum is divided by the sum of these five weights ({@code 3.0}); the native table is
     * declared with a sixth, unused, implicitly-zero entry that {@code smpl_sum_vec(smpl_vuv_weights, 5)} never
     * reads.
     */
    private static final float[] VUV_WEIGHTS = {1.0f, 0.5f, 0.5f, 0.7f, 0.3f};

    /**
     * Sum of the five feature weights, the native {@code smpl_sum_vec(smpl_vuv_weights, 5)}.
     *
     * <p>Computed as the native left-to-right running sum {@code (((w0 + w1) + w2) + w3) + w4}; for the constant
     * weight table this is exactly {@code 3.0f}.
     */
    private static final float WEIGHTS_SUM = sumVec5(VUV_WEIGHTS);

    /**
     * Hysteresis copy of the previous frame's voicing strength, the native {@code VUV_Mode.voicing_prev}.
     *
     * <p>Set each frame to {@code tanhf(3 * voicing_strength)} of the frame just classified, optionally divided
    ...
    /**
     * The previous frame's last-subframe pitch lag, the native {@code VUV_Mode.last_lag_prev}.
     *
     * <p>Compared against the current frame's {@code lags[0]} to detect a lag jump that decays the hysteresis
     * carry-over; a non-positive value (the initial state) disables the decay for the first frame. Zero on
     * construction and after {@link #reset()}.
     */
    private float lastLagPrev;

    /**
     * Smoothed low-band background energy floor, the native {@code VUV_Mode.nrg_lo_bgn}.
     *
    ...
    /**
     * Smoothed high-band background energy floor, the native {@code VUV_Mode.nrg_hi_bgn}.
     *
     ...
     * zero, the native zero-initialized {@code VUV_Mode}.
     */
    SignalModeClassifier() {
    }

    /**
     * Restores the classifier to its freshly-constructed state, the native zero-initialized {@code VUV_Mode}.
     *
    ...
    /**
     * Classifies one frame and returns its voicing strength, the native {@code smpl_get_signal_mode}.
     *
     * <p>Combines the five soft voicing features into a normalized, biased, hysteresis-smoothed strength and
     * advances the internal {@code VUV_Mode} state: it updates the smoothed background energy floors on inactive
     * frames, decays and rescales the hysteresis carry-over, stores this frame's {@code tanh}-shaped strength as
     * the next frame's carry-over, and records this frame's last lag. The caller treats a strictly positive
     * return on an active-voice packet as a voiced frame and any other return as unvoiced.
     *
     ...
     *
     * @param pitchCorr    the open-loop pitch correlation, the native {@code pitchcorr} from
     *                     {@link OpenLoopPitch.Result#pitchCorr()}
     * @param lags         the per-subframe pitch lags of this frame, the native {@code lags} from
     *                     {@link OpenLoopPitch.Result#lags()}
     * @param avgLag       the dominant-subframe average lag, the native {@code avg_lag} from
     *                     {@link OpenLoopPitch.Result#avgLag()}
     * @param harmStrength the spectral harmonicity of the average lag, the native {@code harm_strength} from
     *                     {@link OpenLoopPitch.Result#harmStrength()}
     * @param f2           the LPC power spectrum of this frame, {@value #F2_LEN} bins, the native {@code F2}
     * @param spActProb    the SILK speech-activity probability in {@code [0, 1]}, the native {@code sp_act_prob}
     *                     from the {@link Vad} result for this frame
     * @return the voicing strength; positive on this active-voice path means voiced, the native return value
     */
    ...
    /**
     * Computes the low-band spectral-tilt energy, the native low-frequency accumulation loop of
     * {@code smpl_get_signal_mode} over bins {@code [2, TRANSITION_IX)}.
     *
     * <p>Each bin contributes {@code ((TRANSITION_IX - i) * (i + 3)) * F2[i]}, with the integer factors
     * converted to float in lane and multiplied before the spectrum value, the order the {@code -Ofast} build
     * emits. The {@code -mavx} build vectorizes the {@code 80}-bin span {@code [2, 81]} into a four-lane
     * packed-single accumulator (lane {@code k} summing bins {@code 2 + k, 6 + k, ...}), reduces the lanes as
     * {@code (lane0 + lane2) + (lane1 + lane3)} after folding in the two-wide tail bins {@code 82} and
     * {@code 83}, and finishes with the scalar bin {@code 84}; this reproduction follows that schedule exactly
     * so its last float bit matches the native object.
     *
     * @param f2 the LPC power spectrum, {@value #F2_LEN} bins
     * @return the low-band tilt energy, the native {@code nrg_lo}
     */
    ...
    /**
     * Computes the high-band spectral-tilt energy, the native high-frequency accumulation loop of
     * {@code smpl_get_signal_mode} over bins {@code [TRANSITION_IX, SMPL_F_LEN)}.
     *
     * <p>Each bin contributes {@code ((i - TRANSITION_IX) * (i + 3)) * F2[i]}, with the integer factors
     * converted to float in lane and multiplied before the spectrum value, the order the {@code -Ofast} build
     * emits. The {@code 172}-bin span {@code [85, 256]} is an exact multiple of four, so the {@code -mavx} build
     * vectorizes it into a four-lane packed-single accumulator (lane {@code k} summing bins {@code 85 + k,
     * 89 + k, ...}) with no scalar tail, and the lanes reduce as {@code (lane1 + lane3) + (lane0 + lane2)}; this
     * reproduction follows that schedule so its last float bit matches the native object.
     *
     * @param f2 the LPC power spectrum, {@value #F2_LEN} bins
     * @return the high-band tilt energy, the native {@code nrg_hi}
     */
    ...
    /**
     * Maps an average lag to the short-lag sigmoid feature, the native
     * {@code smpl_sigmoid(0.25f * (38.0f - avg_lag))}.
     *
     * <p>Reproduces {@code smpl_sigmoid} including its overflow guards: the sigmoid argument
     * {@code x = 0.25 * (38 - avgLag)} is clamped so {@code x > 80} returns {@code 1} (very short lags) and
     * {@code x < -80} returns {@code 0} (very long lags), and otherwise the value is
     * {@code 1 / (1 + expf(-x))}, with {@code -x} formed as {@code 0.25 * (avgLag - 38)} exactly as the build
     * does. The classifier negates this sigmoid to form the short-lag feature; the negation is folded into the
     * final weighted sum as a subtraction, so this method returns the un-negated sigmoid value.
     *
     * @param avgLag the dominant-subframe average lag, the native {@code avg_lag}
     * @return the short-lag sigmoid value in {@code [0, 1]}, the native {@code smpl_sigmoid} result before the
     * sign flip
     */
    ...
    /**
     * Computes the inverse sigmoid of a probability, the native static {@code smpl_inv_sigmoid}.
     *
     * <p>Returns {@code -logf((1 / x) - 1)}, the logit of {@code x}. The caller only ever passes
     * {@code 0.1 + 0.75 * c} for a correlation {@code c} clamped to {@code [0, 1]}, so the argument lies in
     * {@code [0.1, 0.85]} and the logarithm is always defined. The negation is computed here; the build folds it
     * into the final weighted sum as a subtraction, which is bit-equivalent.
     *
     * @param x the probability in {@code (0, 1)}
     * @return the logit of {@code x}, the native {@code corr_strength} contribution before weighting
     */
    ...
    /**
     * Computes a single-precision exponential, the native {@code expf} linked into the short-lag sigmoid.
     *
     ...
     * nearest integer {@code k} (round-half-to-even, the SSE {@code cvtpd2dq} conversion), and decomposes
     * {@code k = 64 m + idx}; the result is {@code table[idx] * (1 + r + r^2 (r/6 + 1/2)) * 2^m} with reduced
     * argument {@code r = x - k * (ln2 / 64)} and {@code table} the 64-entry {@link #EXP_TABLE} of fractional
     * powers of two, finally rounded back to {@code float}. The caller only reaches this on arguments in
     * {@code [-80, 80]}, so the overflow and underflow tails of the native routine are not exercised; this port
     * defers to {@link Math#exp(double)} for any argument outside the table-reduction range as a guard.
     *
     * @param x the argument, the native {@code 0.25 * (avg_lag - 38)}
    ...
    /**
     * Computes a single-precision natural logarithm, the native {@code logf} linked into
     * {@code smpl_inv_sigmoid}.
     *
     ...
     * {@code e}; the reduced argument {@code r = recip[idx] * (idx_value - mantissa)} drives the cubic
     ...
     * {@code 1/16} of one take a dedicated {@code atanh}-style series in {@code s = (x - 1) / (x + 1)}. The
     * caller only reaches this on the strictly positive {@code (1 / p) - 1} for {@code p} in {@code [0.1, 0.85]}.
     *
    ...
    private static float logf(float x) {
        ...
        if (absDiff < 0.0625f) {
            ...
            //  frame in 72000 under near-one-heavy fuzzing and zero frames under realistic speech features.
    ...
    /**
     * Computes a single-precision base-2 logarithm, the native {@code log2f} linked into the hysteresis
     * lag-jump test.
     *
     ...
     * {@code [sqrt(1/2), sqrt(2))} and an exponent {@code e}, evaluates the {@code atanh}-style series in
    ...
    /**
     * Sums the first five feature weights left to right, the native {@code smpl_sum_vec(x, 5)}.
     *
     * <p>Accumulates as {@code (((x0 + x1) + x2) + x3) + x4}, the native running-sum order; for the constant
     * weight table the result is exactly {@code 3.0f}.
     *
    ...
    /**
     * Mantissa table of the fractional powers of two used by the {@code tanhf} exponential path, the native
     * {@code tanhf} first lookup table.
     *
     * <p>Entry {@code i} is {@code 2^(i/32)} rounded to a fourteen-bit mantissa, paired with the correction term
     * in {@link #TANHF_TABLE_LOW}; together they give the reduced-range scale factor of the table-based
     * single-precision exponential. The bit patterns are reproduced verbatim from the platform runtime so the
     * exponential, and therefore {@link #tanhf}, matches the build's linked {@code tanhf} bit for bit.
     */
    ...
    /**
     * Correction table for the fractional powers of two used by the {@code tanhf} exponential path, the native
     * {@code tanhf} second lookup table.
     *
     * <p>Entry {@code i} is the low part of {@code 2^(i/32)}, added to {@link #TANHF_TABLE_HIGH} entry {@code i}
     * to recover the scale factor to full single-precision accuracy. The bit patterns are reproduced verbatim
    ...
    /**
     * Mantissa table of fractional powers of two for the {@code expf} reduction, the native {@code expf} lookup table.
     *
     * <p>Entry {@code i} is the {@code double} bit pattern of {@code 2^(i/64)}, recovered with {@link Double#longBitsToDouble(long)} and multiplied by the reduced-range polynomial of {@link #expf}. The bit patterns are reproduced verbatim from the platform runtime.
     */
     ...
     *
     * <p>Entry {@code idx} is the {@code float} bit pattern of the reciprocal that scales the reduced mantissa argument of {@link #logf}, recovered with {@link Float#intBitsToFloat(int)}. The bit patterns are reproduced verbatim from the platform runtime.
     */
     ...
     *
     * <p>Entry {@code idx} is the {@code float} bit pattern of the high part of the logarithm of the reduction point of {@link #logf}, recovered with {@link Float#intBitsToFloat(int)} and accumulated with the high part of {@code e * ln2}. The bit patterns are reproduced verbatim from the platform runtime.
     */
     ...
     *
     * <p>Entry {@code idx} is the {@code float} bit pattern of the low part of the logarithm of the reduction point of {@link #logf}, recovered with {@link Float#intBitsToFloat(int)} and accumulated with the low part of {@code e * ln2}. The bit patterns are reproduced verbatim from the platform runtime.
     */
    ...
    /**
     * Computes a single-precision hyperbolic tangent, the native {@code tanhf} linked into the hysteresis state
     * update.
     *
     * <p>The build links the platform single-precision {@code tanhf}, whose result is stored back as the next
     * frame's hysteresis carry-over ({@link #voicingPrev}); a one-bit error there propagates through the
     * hysteresis chain and can flip a later frame's voicing decision, so this is a verbatim port of that routine
     * rather than a rounded double-precision tanh, which differs by up to one unit in the last place. The
     * routine uses a sign-folded {@code |x|} and three magnitude regions:
     * <ul>
     ...
     * arithmetic of the table index and the power-of-two exponent split uses the same arithmetic shifts and
     * sign biases.
     *
     * @param x the argument, the native {@code 3 * voicing_strength}
