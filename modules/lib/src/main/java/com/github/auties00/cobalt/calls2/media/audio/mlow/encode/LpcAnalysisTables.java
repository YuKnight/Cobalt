...
/**
 * Native-exact constant tables for the MLow encoder LPC analysis front-end, the captured output of
 * {@code smpl_create_lpc_tables} and {@code smpl_create_lpc_windows} in {@code smpl_lpc.c}.
 *
 * <p>The native build computes these once at startup: the brute-force inverse-DCT cosine rows
 * ({@code Cdif}, {@code Csumdiff}, {@code Csumsum}) and the analysis windows (the 20 ms sine taper and the
 * short and long cosine tapers). They are pure constants of the build, not signal-dependent, so they are
 * captured here rather than regenerated.
 *
 * <p>Why captured and not generated. The rows are {@code cos(omega) * scale} with {@code omega} accumulated
 * by {@code fmod}; the tapers are {@code sinf}/{@code cosf} of a fixed angle. The native build links the
 * mingw-w64 C library, whose {@code cos}/{@code sin}/{@code cosf}/{@code sinf} are not correctly rounded and
 * disagree with the JDK {@link java.lang.Math} transcendental functions by up to one unit in the last place
 * on a few entries. Those single-bit differences are harmless in isolation, but the brute-DCT dot product
 * that recovers the autocorrelation has heavy cancellation at the higher lags, which amplifies a one-bit
 * table error into a part-in-{@code 1e7} autocorrelation error and then, through the ill-conditioned
 * Levinson recursion, into a line-spectral-frequency shift large enough to flip a quantizer index on
 * resonant frames. Capturing the exact native tables removes that source of divergence entirely; the values
 * are written as hexadecimal float literals so the bytes are identical to the native ones.
 *
 * <p>Scope is the SMPL 16 kHz / 60 ms / mono path: prediction order {@code 16}, FFT length {@code 512}, and
 * 20 ms frames. This holder is package-private to the analysis front-end.
 */
final class LpcAnalysisTables {
    /**
     * Flat brute-DCT cosine table: eight {@code Cdif} rows, then four {@code Csumdiff} rows, then four
     * {@code Csumsum} rows, each row {@code 128} {@code double} entries.
     */
    static final double[] DCT = dct();

    /**
     * Assembles {@link #DCT} from its chunk initializers.
     *
     * @return the flat brute-DCT cosine table
     */
    private static double[] dct() {
        double[] t = new double[2048];
        dct0(t);
        dct1(t);
        dct2(t);
        dct3(t);
    ...
    /**
     * The 20 ms leading sine-taper analysis window, 264 entries.
     */
    ...
    /**
     * The long trailing cosine-taper analysis window, 64 entries.
     */
    ...
    /**
     * The short trailing cosine-taper analysis window, 32 entries.
     */
