 ...
 * inter-arrival-delay samples and applying an adaptive over-use detector.
 *
 ...
 * {@link #K_UP} while over-using to suppress repeat triggers and falling with gain {@link #K_DOWN}
 ...
 *
 * @implNote This implementation ports WebRTC's {@code TrendlineEstimator}
 * ({@code modules/congestion_controller/goog_cc/trendline_estimator.cc}) and the over-use-detector
 * threshold adaption, reached in the wa-voip engine through {@code bwe/bwe_webrtc_delay_based.cc}
 * (fn6193), which parses the {@code WebRTC-Bwe-TrendlineEstimatorSettings} field-trial
 * ({@code window_size}, {@code k_up}, {@code k_down}, {@code overusing_time_threshold},
 * {@code smoothing_coef}, {@code override_overuse_threshold}). The captured {@code voip_settings} blob
 * (re/calls2-spec/captures) sets {@code delay_based_bwe_aimd_adpt_thresh_exp=1} and
 * {@code delay_based_bwe_bitrate_estimator_enabled=1} but does not carry the trendline field-trial
 * string, so the numeric constants are the upstream WebRTC defaults inferred from the field-trial
 * names and match the existing {@code call.rtp.congestion.TrendlineEstimator} this class supersedes.
 */
public final class TrendlineEstimator {
     ...
     *
     * <p>Raising the threshold while over-using suppresses repeated {@link BandwidthUsage#OVERUSING}
     ...
     *
     * <p>Lowering the threshold when the link is not over-using recovers detector sensitivity.
     */
     ...
     *
     * <p>This low-pass-filters single-spike excursions so they do not trigger an over-use
     * classification.
     */
    ...
    /**
     * Cumulative time, in milliseconds, the detector has spent in over-use-candidate territory.
     *
     * <p>Reset whenever the modified trend leaves the over-threshold band; compared against
     * {@link #OVERUSING_TIME_MS} to gate the over-use classification.
     */
     ...
     *
     * <p>The over-use confirmation requires the current modified trend to be at least this value, so a
     * trend that has begun to reverse (a current modified trend below the previous one) is filtered out
     * and does not declare an over-use. Initialized to {@code 0.0} so the first excursion is not blocked
     * by a stale prior.
     */
    ...
    /**
     * Classifies the link as over-using, under-using, or normal and updates {@link #state}.
     *
     * <p>A modified trend above {@link #threshold} accrues {@link #overusingTimeMs} (capped per step
     * at {@link #MAX_TIME_DELTA_MS}) and increments {@link #overuseCounter}; an over-use is declared
     ...
     * single-spike noise from triggering it. The not-just-reversed check compares the current modified
     * trend against {@link #prevModifiedTrend}, the value from the prior call, so a current trend below
     * the previous one (a reversing trend) is filtered out, the WebRTC {@code kBwOverusing} reversal
     * condition. A modified trend below the negated threshold yields under-use, anything in between
     * yields normal, and both reset the over-use accumulators.
     *
     ...
     *
     * <p>Clears the sample window, the smoothed and accumulated delays, the over-use accumulators, and
