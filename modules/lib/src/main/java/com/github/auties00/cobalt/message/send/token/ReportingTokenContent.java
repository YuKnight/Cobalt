...
/**
 * Builds the deterministic franking content over which the reporting-token HMAC is
 * computed.
 *
 * <p>The reporting token authenticates a <em>sparse</em> copy of a message's serialised
 * protobuf, not the full payload: only the field numbers whitelisted by
 * {@link ReportingConfig} for the current sender version are retained, recursively.
 * Computing the HMAC over anything other than this sparse copy makes the server-side
 * verification fail with a {@code reporting-token-validation-failure}, even when every
 * other field of the stanza is correct.
 *
 * <p>The whitelist is shipped inline in the WA Web bundle as a base64-encoded protobuf
 * ({@link #REPORTING_TOKEN_CONFIG_BASE64}); this class decodes it lazily via
 * {@link ReportingConfigSpec#decode(byte[])} and caches the result. The walker builds
 * an intermediate tree of {@link KeptNode} carriers, summing each node's emit size on
 * the way up, then materialises the output into a single pre-sized {@code byte[]}.
 */
...
public final class ReportingTokenContent {
    ...
    /**
     * Sorts kept nodes by ascending field number to match WA Web's canonical emission
     * order.
     */
    ...
    /**
     * The base64-encoded {@code Config} protobuf shipped inline in the WA Web bundle.
     * Decoded against {@link ReportingConfigSpec} on first use.
     */
    ...
    /**
     * Cache keyed by sender version. The decoded config does not vary with the version,
     * but pre-pruning is unnecessary because the version filter is cheap and consulted
     * at every node.
     */
    ...
    /**
     * Returns the decoded reporting-token configuration for the given sender version.
     *
     * <p>The current implementation returns the same {@link ReportingConfig} for every
     * version because the version-pruning step is fused into the byte walker. The
     * parameter is preserved to keep the API symmetric with WA Web and to support
     * future version-specific pruning if needed.
     *
     * @param senderVersion the {@code rt_sender_reporting_token_version} currently in
     *                      effect
    ...
    /**
     * Computes the deterministic franking content over which the reporting-token HMAC
     * must be applied.
     *
     * <p>Walks the protobuf wire format of {@code messageBytes} field-by-field, keeps
     * only the fields whitelisted by the config for {@code senderVersion}, recurses on
     * sub-messages, sorts the retained fields by ascending field number, and
     * concatenates the raw bytes of each retained field. The output is bound to the
     * encoded layout of the source: changing the order or wire encoding of a kept
     * field changes the HMAC, which is desired because the server replays the same
     * algorithm on its own copy of the message and rejects mismatches.
     *
     * @param messageBytes  the serialised {@code MessageContainer} protobuf whose
     *                      franking content is to be computed
     * @param senderVersion the {@code rt_sender_reporting_token_version} currently in
     *                      effect
    ...
    /**
     * Recursively walks {@code src[start, end)}, appends every whitelisted field as a
     * {@link KeptNode} into {@code out}, and returns the total number of bytes the
     * appended nodes will emit.
     *
     * @param src           the raw protobuf bytes containing the sub-region to walk
     ...
     * @param topRules      the top-level rule map, consulted when a kept field carries
     *                      {@code isMessage = true}
     * @param senderVersion the version that filters out rules whose
     *                      {@code minVersion}/{@code maxVersion} bracket excludes it
     * @param out           accumulator for the kept nodes at this level
     * @return the total emit size of the nodes appended at this level
     ...
     *
     * <p>The rule is excluded when {@code version < minVersion} or
     * {@code version > maxVersion}, with {@code maxVersion} defaulting to positive
     * infinity. {@code notReportableMinVersion} acts as the inclusive lower bound for
     ...
     * {@link ReportingConfig} the first time it is requested and returns the cached
     * instance thereafter.
     *
    ...
    /**
     * Advances {@code cursor} past one wire-format value of type {@code wireType}.
     *
     ...
     * @throws IllegalArgumentException if the value is malformed or the wire type is
     *                                  unsupported
     */
     ...
     * @param end    exclusive end of the sub-region
     * @return the decoded value paired with the position immediately after the varint
     * @throws IllegalArgumentException if the varint is truncated or longer than 10
     *                                  bytes (overflowing a 64-bit unsigned integer)
     */
     ...
     * LEB128-encoded.
     *
     * <p>Constants are folded so the lookup is a chain of compares rather than a loop.
     * Negative values are treated as 10-byte sign extensions to match the protobuf wire
     * format, even though the walker never produces negative inner-message lengths.
     *
    ...
    /**
     * Writes {@code value} as a LEB128-encoded varint into {@code out} starting at
     * {@code cursor}.
     *
    ...
    /**
     * Carrier returned by {@link #readVarInt}: the decoded value paired with the cursor
     * advanced past the varint.
     *
    ...
    /**
     * A field that survived the whitelist filter, plus the logic to write it into the
     * output buffer.
     */
    private sealed interface KeptNode {
        ...
        /**
         * Writes this node's bytes into {@code out} starting at {@code cursor} and
         * returns the new cursor.
         *
         * @param src    the original source bytes (verbatim slices are copied straight
         *               from here)
         * @param out    destination buffer (must be pre-sized)
         * @param cursor the offset at which to start writing
         * @return the offset immediately after the written bytes
        ...
        /**
         * A field that is kept verbatim: the writer copies the tag plus the value bytes
        ...
        record Leaf(int fieldNumber, int sourceStart, int length) implements KeptNode {
            /**
             * Copies {@link #length} bytes starting from {@link #sourceStart} into
        ...
        /**
         * A length-delimited field whose body has been recursively pruned. The original
         * tag is reused verbatim, but the inner length-prefix is freshly emitted because
         * the pruned body's size differs from the source body's size.
         *
         * @param fieldNumber the protobuf field number used for sort order
         * @param tagStart    the offset into the source where the original tag bytes
         *                    begin
         * @param tagLength   the number of bytes spanning the original tag
         * @param innerSize   the total byte size of {@code children} when written
        ...
        record Branch(int fieldNumber, int tagStart, int tagLength,
            ...
            /**
             * Copies the original tag, writes a fresh length prefix for the pruned
             * body, and recursively writes each kept child.
             *
             * @param src    the original source bytes
             * @param out    the destination buffer
             * @param cursor the offset at which to start writing
