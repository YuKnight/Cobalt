 ...
 * record reproduces that envelope. It is the unit of exchange between {@link SctpDtlsBridge} (which
 ...
 * and {@link #DTLS_SOCKADDR_LEN} the engine requires; a mismatch is rejected the same way the native
 * {@code transport_send_scpt_dtls_packet} validator rejects a malformed envelope.
 *
 ...
 * @implNote This implementation reproduces the {@code DtlsPacketEnvelope (sockaddr_conn)} structure from
 * {@code transport_send_scpt_dtls_packet} (fn5064) in {@code transport/wa_transport.cc} of the wa-voip
 * WASM module {@code ff-tScznZ8P}: that function accepts a DTLS packet only when its {@code sockaddr_conn}
 * reports {@code family == 2} ({@code param2[0]}) and {@code addr_len == 0x10} ({@code param2[1]}), then
 * reads the DTLS record pointer from {@code param2[10]} and its length from {@code param2[0xb]} and routes
 * it over the elected relay. The fixed-value fields are kept as record components rather than implicit
 * constants so an envelope decoded from a foreign source round-trips its declared discriminator and the
 * {@link #of(byte[])} factory can validate it; {@link #relayPacket()} returns just the payload because the
 * relay transport ships the DTLS record bytes verbatim with no sockaddr prefix.
 */
public record DtlsPacketEnvelope(int family, int addressLength, byte[] payload) {
     ...
     *
     * @implNote This implementation uses {@code 2}, the {@code DTLS_SOCKADDR_FAMILY} constant the native
     * {@code transport_send_scpt_dtls_packet} (fn5064) requires in {@code param2[0]}; the voip engine
     * overloads the {@code AF_CONN}-style family value {@code 2} as its "this is a DTLS record" tag.
     */
     ...
     *
     * @implNote This implementation uses {@code 0x10}, the {@code DTLS_SOCKADDR_LEN} constant the native
     * {@code transport_send_scpt_dtls_packet} (fn5064) requires in {@code param2[1]}: the synthetic
     * address occupies sixteen bytes regardless of the underlying family.
     */
     ...
     *
     * <p>This is the factory the Web-P2P {@link SctpDtlsBridge} uses to envelope an outbound DTLS record
     * before it reaches the relay datagram sink: the {@link #DTLS_SOCKADDR_FAMILY} and
     * {@link #DTLS_SOCKADDR_LEN} values are supplied automatically, so the caller passes only the record.
     *
