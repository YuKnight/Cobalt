 ...
 * maps onto narrowband through fullband. The wa-voip engine reads the current ceiling with the
 * get-maximum-bandwidth control and applies a new one with the set-maximum-bandwidth control.
 *
 * @implNote This implementation reproduces {@code wa_opus_set_max_bandwidth} (fn6260) of the wa-voip
 * WASM module {@code ff-tScznZ8P}, which computes {@code OPUS_BANDWIDTH_NARROWBAND + level} where
 * {@code OPUS_BANDWIDTH_NARROWBAND == 0x44d == 1101} and {@code level} runs {@code 0..4} for
 * NB/MB/WB/SWB/FB, giving codes {@code 1101..1105}. The level-to-bandwidth mapping is the libopus
 * constant ordering, not the wa {@code opus_default_attr} bandwidth-enum column ({@code 0/2/3/4}),
 * which is a distinct per-sample-rate index resolved by {@link OpusDefaultAttr}.
 */
