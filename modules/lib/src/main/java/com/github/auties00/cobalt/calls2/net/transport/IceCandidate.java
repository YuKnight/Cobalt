 ...
 * server-reflexive, peer-reflexive, or relayed), the wire {@link Protocol protocol} (UDP or TCP), and
 * the {@code u32} candidate priority RFC 8445 assigns. The priority is computed from the type
 * preference, the local preference, and the component id by
 * {@link #computePriority(int, int, int)} when gathering candidates.
 *
 * @implNote This implementation models the ICE candidate of {@code wa_transport_p2p.cc} from the
 *           wa-voip WASM module {@code ff-tScznZ8P} on the Web-P2P interop path. The priority formula is
 *           the RFC 8445 one ({@code (2^24)*typePref + (2^8)*localPref + (256 - componentId)}); the
 *           proprietary relay path uses {@link com.github.auties00.cobalt.calls2.signaling.RelayCandidate}-style endpoints rather than ICE
 *           candidates.
 */
