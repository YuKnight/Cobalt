 ...
 * id-correlated {@link LinkedWhatsAppClient#sendNode(NodeBuilder)}; that call blocks the calling
 ...
 * contract and is rejected with an {@code error} code the same way. {@link #parse(Node)} turns that
 * return node into the typed outcome the engine branches on, so neither the offer sender nor the
 ...
 * {@code <relay>} child is then re-parsed with {@link RelayInfo#of(Node)} so the outcome carries the
 * calls2 relay model the transport layer consumes, rather than the legacy relay {@link CallAck#relay()}
 * yields. The whole layer is pure-Java glue: a Cobalt {@link Node} is already the parsed tree, so there
 * is no node-to-voip-node bridge to cross.
 *
 ...
 * the {@link LinkedWhatsAppClient#sendNode(NodeBuilder)} return value. The accept NACK classification by
 * {@code error} code mirrors {@code stanzas/basic.cc} ("Accept ACK: error code = %d. Treating as an
 * Accept NACK"). The relay re-parse delegates to {@link RelayInfo#of(Node)}, which ports
...
public final class CallAckParser {
     ...
     * {@link LinkedWhatsAppClient#sendNode(NodeBuilder)} for the offer or accept envelope. The envelope
     ...
     * into the result. A return node that the shared parser does not classify as a call ack (for
     ...
     *
     * @param ack the {@code <ack>} node returned by the server for the offer or accept
     * @return the parsed {@link CallAckOutcome}, or {@link Optional#empty()} when the node is not a
     *         call-class ack
     * @throws NullPointerException     if {@code ack} is {@code null}
     * @throws IllegalArgumentException if the node tag is not {@code "ack"}
     */
    public static Optional<CallAckOutcome> parse(Node ack) {
     ...
     *
     * <p>Reads the single {@code <relay>} child and runs it through {@link RelayInfo#of(Node)} so the
    ...
    /**
     * Convenience that parses the {@code <ack class="call">} return node and yields only its relay
     * block.
     *
     ...
     * {@link #parse(Node)} instead and branch on {@link CallAckOutcome#isAck()}.
     *
     * @param ack the {@code <ack>} node returned by the server for the offer or accept
     ...
     * @throws IllegalArgumentException if the node tag is not {@code "ack"}
     */
    public static Optional<RelayInfo> parseRelayBlock(Node ack) {
